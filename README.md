...
- chip_strategies.py
  籌碼策略模組，負責四套法人/大戶策略、共同硬篩、法人日序列抓取、TDCC 週資料快取與週報補發狀態管理。
- data_fetcher.py
...
- /scan
  顯示互動式策略選單，可選擇財報營收選股、四套籌碼策略或全部執行。
- /export 代碼
  匯出單一股票的 Excel 資料包。
- /morning
...
- 籌碼策略一、二會先讀 .cache/chip_daily；缺資料才抓 TWSE / TPEx 官方資料，TWSE 被 307 或其他安全頁擋住時，改用 FinMind TaiwanStockInstitutionalInvestorsBuySell 逐檔補資料，成功後寫回快取。
...
- 若 TWSE T86 或 MI_QFIIS 發生 307、安全頁、逾時等錯誤，該資料源會進入 30 分鐘冷卻，冷卻期間不重複撞同一端點。
...
- 每日台灣時間 18:00 自動推播策略一、策略二、策略三
- 每週六 12:00 自動推播策略四
- Telegram 訊息有自動分段機制，避免單則超過平台長度限制

### 籌碼排程防呆與補發

- 每日 18:00 的籌碼排程會檢查最新交易日是否為今天，若不是則直接略過，不發送舊資料。
- 每週六 12:00 的策略四若因維護或重啟漏發，系統會在週一啟動時檢查是否已發送過該週週報。
- 若尚未發送，會立即補送一次策略四報告，並把結果寫入 .cache/chip_strategy_state.json。
...
- .cache/gross_margin.json
  毛利率快取，12 小時
- .cache/chip_daily
  籌碼策略一到三的每日法人資料快取；每個交易日一個 CSV，包含外資買賣超、投信買賣超與可取得的外資持股比例。讀取時先用本機快取，缺漏才補抓官方或 FinMind。
- .cache/tdcc
  籌碼策略四的 TDCC 週快照快取，最多讀取最近 8 週
- 資料源冷卻
  TWSE T86 或 MI_QFIIS 失敗後會冷卻 30 分鐘；FinMind fallback 每個交易日最多補 50 檔，並在錯誤時進入冷卻，降低被限流機率。
