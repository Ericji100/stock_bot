...
`config/prompts/` 與 `config/scoring/` 暫時保留作舊版相容或備份，不是主要維護位置。
...
> **注意**：此段為 2026-05-07 歷史紀錄；現行主要 prompt 維護位置已改為 `prompt/`，`config/prompts/` 與 `config/scoring/` 僅保留作舊版相容或備份。
