...
| `/scan` 全市場選股 | 穩定可用 | 技術、財報、籌碼、精選、老蕭策略與指定日期掃描 |
...
| 單股深度研究 | `/research 2330` | 手動輸入預設深度研究，等同 `/research 2330 --deep` |
| 明確指定深度研究 | `/research 2330 --deep` | 較完整的外部證據與投研報告 |
...
- `/scan` 不帶日期時，先選擇 9 種選股方式，再選擇「最新日期」或「指定日期」。
- 「最新日期」會使用最近可用交易日；若今天是週末或休市日，會自動往前找最近交易日。
- 「指定日期」與 `/scan 2026-05-22` 會照輸入日期執行，不自動改成最新交易日。
- 「全部執行」會包含財報、法人籌碼、技術面、精選與老蕭選股，並保存合併候選名單供後續流程使用。
- 老蕭策略也可由 `/radar --source laoxiao` 當成 Radar 候選來源；Radar 的預設 `combined` 暫不自動混入此策略。

### 技術指標價格基礎與參數

- 股票日線的選股、MACD 動能策略（A–D）、雙均線結構策略、KD 動能均線策略、股票圖表、監控與 Radar 技術快照，共用 `technical_indicator_service.py` 的計算規則。
- 日線 OHLC 使用完整還原權值。系統以來源 OHLC 與 `Adj Close` 的比例調整開高低收；Telegram 顯示的即時價或最新成交價仍維持來源報價尺度。
- 指定歷史日期時，還原因子會以該日期資料截點的最後一日重新基準化，不使用截點之後的除權息結果，避免歷史重播偷看未來資料。
- MACD 固定使用 `21 / 55 / 55`：DIF 為 EMA21 減 EMA55，訊號線為 DIF 的 EMA55，柱體為 DIF 減訊號線。
- 策略 A 的前波低點取已完成紅柱前、緊鄰該紅柱的連續綠柱（`MACD_HIST < 0`）全段最低價；前波高點取該紅柱全段最高價。零柱不算綠柱，也不跨越零柱取更早的低點；前波漲幅及回檔比例依此波段計算。
- KD 固定使用 `9 / 9 / 55`：RSV 取 9 日區間；K 採 `K=(8/9)×前 K+(1/9)×RSV`；D 採 `D=(54/55)×前 D+(1/55)×K`。不可用 `ewm(span=9/55)` 取代此平滑公式。
- 股票分鐘線與期貨圖表沒有公司除權息，維持來源價，但共用相同 MACD、KD 參數與公式。
- 技術日線快取旁會保存指標版本與價格來源基礎。舊快取若沒有有效 `Adj Close`，技術掃描會重新取得可還原資料；資料源確實無還原欄位時才使用來源價 fallback。
- 回歸基準：美律 2026-09-15 不應出現 KD 黃金交叉，2026-09-16 才交叉；中鼎 2026-09-16 的還原權值 MACD 應接近三竹顯示的 DIF `0.90`、訊號線 `0.64`。

### 策略 B：強勢紅柱回測突破

- B1、B2 都要求訊號日 `MACD_HIST > 0`；B1 另要求收盤高於昨日收盤及今日 MA21，B2 不套用這兩項門檻。
- **B1｜當日低點碰觸 MA5/MA13/MA21 後收復**：今日低點碰到均線，收盤站回同一條均線；不要求昨日收盤在該線下。沿用原有的 MA13、MA21 優先順序，再檢查 MA5；一檔在 B1 只輸出一筆，股票摘要標示命中均線。
- **B2｜紅柱期間收盤突破 MA5/MA13/MA21**：訊號日為 MACD 紅柱；對每條有效均線，昨日收盤不高於昨日均線、今日收盤高於今日均線，即在突破當日觸發。不限定之前跌破多久，也不要求今日收盤高於昨日或 MA21、不檢查先前 MA60。昨日已站上同一條均線者不重複通知；同日突破多條均線只輸出一筆，摘要列出所有命中均線。B1 的盤中碰線收回與 B3 的回測前高判斷獨立保留，同一股票可能同時符合不同分組。
- B3 維持既有 MA13／MA21 回測後突破前高判斷，未納入 MA5。

### 策略 D：動能背景短線轉強

- **D1｜零軸上短均線收復**：今日 `DIF > 0`；若恰好等於 0，最近一個非零 DIF 從上方到達 0 也歸 D1。不要求收盤高於 MA60／MA105，不要求近期紅柱。
- **D2｜零軸下短均線收復**：今日 `DIF < 0`；若恰好等於 0，最近一個非零 DIF 從下方到達 0 也歸 D2。另須前 5 個交易日曾有 `MACD_HIST > 0`；不要求收盤高於 MA60／MA105。若找不到先前非零 DIF，零值不歸 D1／D2。
- D1／D2 共用 MA5、MA13、MA21 的收復判斷：昨天收盤不高於該日均線、今天收盤高於今日均線；或今天出現紅 K 長下影（下影長度大於實體兩倍）、盤中低點碰線並收盤站回，且目前連續站上該線期間尚無較早的突破或合格長下影回踩。只有長下影但未碰線收復不入選。每條均線獨立判斷，同天命中多條只列一筆；同條均線在 D2 通知後轉入 D1，但未再跌破並重新收復，不重複通知。標籤與股票註記不加「首次」。
- **D3｜KD 死叉後首次轉強**：不要求收盤高於 MA60／MA105，但保留 `DIF > 0` 或前 5 日曾有紅柱的動能門檻。近 1–3 個交易日曾發生 `K >= D` 轉為 `K < D`，今天出現 `K > D`、紅 K，或收盤高於 MA5／MA13；同一次死叉後若較早交易日已符合轉強條件，今天不重複通知。MACD 紅柱翻綠不構成獨立訊號。
- Telegram 維持「策略 → 分組 → 產業 → 股票」格式，D1／D2 股票後列出命中的均線，必要時附註長下影或零值方向；D2 另標示近期紅柱。同一股票符合不同分組會各列一次，但策略檔數只計一檔。D 訊號保留「高風險短線策略」註記；指定歷史日期只使用截至該日的資料。

### 雙均線結構策略與下游選股

- `/scan` 將原「四大技術策略」標題改為「MACD 動能策略（A–D）」，A～D 內部判斷不變；另設「雙均線結構策略」，依分組、產業、股票顯示。既有正面／負面訊號仍獨立保留。
- **MA5 > MA21**：今日均線排列符合，且股價今日突破或盤中回踩後收復 MA5。依今日收盤相對有效 MA105／MA144 的位置，分成長均線上方、下方或交錯；夾在兩條長均線之間只列一次，不要求同時高於兩條。至少要有一條有效長均線，不以缺值推算位置。
- **MA21 > MA105 或 MA144**：MA21 高於任一條有效長均線即可，且股價今日突破或盤中回踩後收復 MA21。均線排列是當日狀態，不要求兩條均線今天才交叉。
- 股價突破沿用昨日收盤不高於昨日目標均線、今日收盤高於今日均線；盤中回踩收復沿用今日低點低於、收盤高於目標均線。每條目標均線在連續收盤站上期間只通知首次；跌回該線後，未來重新站上才可再通知。同日同股命中 MA5 與 MA21 時只列於 MA21 分組一筆，摘要註明同步命中 MA5。
- Radar 的 `technical` 與 `combined` 候選來源會讀取雙均線結構訊號，保留於技術快照與證據包；舊技術快取缺此欄位時重跑技術掃描並更新快取。雙均線本身不另設固定加分，既有均線技術快照仍按 Radar 原評分規則計算。
- 精選選股現在將 A～D 與雙均線結構一併視為技術候選入口，仍須符合原有至少兩筆營收財報／法人籌碼命中才會入選；純技術命中不直接入選。每檔在精選報告只列一次，列出所有命中理由；評分沿用 Radar 共用核心，不按技術分組數另加分。

### KD 動能均線策略

- 使用共用 KD `9 / 9 / 55` 與還原權值日 K。黃金交叉沿用既有嚴格定義：前一日 `K < D`、當日 `K > D`。
- **K1｜KD 金叉後 MA5 轉強**：最近 1～8 個交易日前曾發生 KD 黃金交叉，訊號日股價突破或盤中回踩收復 MA5。金叉後即使曾再發生 KD 死叉仍可觸發；不要求訊號日 `K > D`。第 1～8 日只使用訊號日前資料，不含訊號日當天金叉。
- **K2｜KD 多方排列突破或收復 MA21**：訊號日 `K > D`，且股價當日突破或盤中回踩收復 MA21；不要求近期剛發生金叉。
- 均線突破沿用「昨日收盤不高於昨日均線、今日收盤高於今日均線」；盤中收復沿用「今日最低價低於今日均線、今日收盤高於今日均線」。同一段連續站上均線期間只通知一次，跌回後重新站上才可再次通知。
- 同日同股同時符合 K1、K2 時只列在 K2 一筆，摘要註明同步符合 K1。Telegram 依「策略 → 分組 → 產業 → 股票」顯示。
- Radar 的 `technical` 與 `combined` 會將 KD 策略納入候選及證據包，但不因 KD 分組額外給固定分數。精選選股將 KD 策略視為技術候選入口，仍須符合既有至少兩筆營收財報／法人籌碼命中才能入選。

### 技術選股去重彙整

- 技術選股原有各分組訊息全部送出後，最後另送「技術選股去重彙整」。彙整一般正面／負面技術訊號、MACD 動能策略 A～D、雙均線結構策略與 KD 動能均線策略。
- 彙整以股票代號去重，同一股票只列一次；不同策略或分組的中文觸發理由全部保留，相同理由只顯示一次。股票依「多方／策略觸發」、「多空訊號並存」、「純風險訊號」分類，再依產業與股票代號排序。
- 彙整只調整 Telegram 顯示，不改任何訊號判斷、技術選股名單、Radar 評分、精選選股門檻或下游候選資料。同一股票在前面的原始策略分組仍可重複出現，方便查看各策略明細。
- 彙整通常作為最後一則獨立訊息；超過 Telegram 安全長度時自動分頁，不截斷股票或觸發原因。
...
| 深入研究單一股票 | `/research 股票代號` 或 `/research 股票代號 --deep` → `/report 股票代號 latest` |
...
| 月營收與財報 | MOPS、FinMind 結構化財報、公開資料、本地快取 | `.cache/` | 季財報統一成 v3 schema，FinMind 可匿名或使用 key，MOPS Plus 保留為備援 |
...
- 候選題材萃取階段會先壓縮大型 structured data；若 `candidate_extract` 逾時或失敗，系統會從 WebFetch / discovery / base sources 以本地關鍵詞規則產生保底候選，避免候選題材直接變成 0。
...
- 若某一批細節補齊逾時或失敗，系統會把該批候選題材轉成本地骨架 action，保留候選階段的來源證據，並由品質正規化補齊待補欄位，避免整批候選題材消失。
...
```text
/topic_maintain
/topic_maintain --deep --model minimax

`/topic_maintain` 是題材庫維護的單一入口。手動不帶參數時預設使用 deep 模式，等同執行大範圍全市場題材維護；若要指定模型可使用 `/topic_maintain --model minimax`。
...
- 10:00：每日自動執行 `/topic_maintain --deep --model minimax`，使用 MiniMax M3 產生題材庫變更包；不會自動套用。排程完成訊息與手動 `/topic_maintain` 使用同一套 Telegram 摘要與操作按鈕，可直接點選查看、確認套用或拒絕。
- 12:30：監控掃描。
- 13:50：午報。
- 17:45：持股報告。
...
- 報告與推播類定時任務使用 `ScheduledTaskService` 序列佇列；若前一個定時任務尚未完成，下一個任務會排隊，等前一個完成後接續執行。
...
定期任務會盡量沿用手動指令的共用核心，但部分排程為了固定推播內容、控制成本或提高容錯，會明確指定參數。程式啟動時 CMD 的「已註冊定時任務」清單會列出這些實際參數。

| 功能 | 手動執行 | 定期任務實際參數 | 差異說明 |
|---|---|---|---|
| 監控掃描 | `/check`，使用 `build_monitor_scan_report()` 預設標題 | 12:30，固定標題與無訊號文案 | 同一核心函式，排程固定推播文案 |
| 午報 | `/noon` / `/tw_market`，單次產生午報 | 13:50，`build_noon_market_report()`，失敗最多重試 6 次，每次間隔 30 分鐘 | 定期任務多 retry，避免資料源短暫未就緒 |
| 題材庫維護 | `/topic_maintain` 只顯示模型選單；選定後固定組成 `--deep --model <model>` | 10:00，固定 `/topic_maintain --deep --model minimax` | 兩者都固定 deep；差異是手動可選模型、排程固定 MiniMax M3 |
| 持股報告 | `/my` 只列出持股清單 | 17:45，`build_portfolio_report()`，依持股報告 retry 規則推播 | 定期推播是完整持股報告，和 `/my` 不同 |
| 新聞整理 | `/news` 可選 latest / 7d / refresh 與 refresh 模型 | 08:45、18:00，`refresh model=minimax`，使用 scheduled latest 與必要時 lightweight refill | 定期任務固定整理新聞庫並推播，手動可自由查詢或更新 |
| 全部選股 | `/scan` 可選單一策略或全部 | 20:30，等同 `/scan` 選項 7，交易日才執行 | 同一選股核心，排程固定全部選股 |
| Radar | `/radar` 預設 `source=combined`，可用參數改來源、日期、模型與 AI 補強數量 | 21:30，固定 `source=technical, ai_top=15, model=minimax, ai_comment=true`，推播摘要最多 15 檔 | 定期 Radar 目前固定技術面來源與短摘要，和手動預設不同 |
| 籌碼快取回補 | 手動通常透過 `/backfill` 進入完整回補流程 | 16:30 / 18:30 今日回補、21:00 完整籌碼快取回補，`force_refresh=false` | 定期任務只暖籌碼快取，不等同完整 `/backfill` |
| 完整資料回補檢查 | `/backfill [date] [force]` 可指定日期與強制刷新 | 每 2 小時，`run_backfill_if_needed(report_date=None, force_refresh=false)` | 同一回補決策核心；定期任務只做自動日期與非強制檢查 |
...
1. 低階資料整理模型固定使用 MiniMax M3；高階正式分析仍依使用者選單或排程設定使用 MiniMax M3 / Gemini / DeepSeek。
...
低階整理現在有共用安全閘門，但目的不是激進節省 MiniMax M3 額度，而是避免 context 爆掉、逾時或格式失敗。低階模型會盡量完整整理資料，真正控制 token 的重點放在高階模型入模。

- 低階模型 prompt 會先估算字數與 token，CMD 會顯示 `prompt=... chars`、`est_tokens=...`、來源數與分段數。
...
### 2026-06-21 /research、Radar 與健檢狀態補強

- `/research --deep` 若高階 prompt 超過共用分段門檻，會改走分段 AI 分析，再做最終整合；若最終整合 timeout、prompt 過大或模型錯誤，系統會先用較小的「精簡整合包」重試一次，仍保留分段結論、風險、反證、資料缺口與來源索引；重試仍失敗時才產出明確標示的 fallback 報告。
- AI 健檢工具會同時列出「指令狀態、AI 狀態、檔案狀態」。`fallback_success` 只代表 fallback 檔案已產出，不代表正式 AI 分析成功；健檢不會把 fallback 報告算成完全通過。
- AI 健檢工具會依指令型態判斷產物：報告型指令檢查 Markdown / HTML / JSON / sources，`/radar` 檢查雷達摘要與 Telegram 訊息，`/news refresh` 檢查 Telegram/output 產物，`/topic_maintain` 檢查變更包或 output，避免把非 HTML 流程誤判成缺檔。
- Radar 早期轉機評分補強「營收 YoY 轉強、價量初動、籌碼評級輔助、財報尚未反映」的觀察分，避免早期波段股只因財報空窗被壓到候選池後段。
- `/topic_maintain` 變更包會寫入 `report_metadata`、`data_source_summary`、`candidate_snapshot` 與 `command_result`，讓題材庫更新也能追蹤來源、模型、日期、變更理由、風險反證與後續驗證。
- `/topic_maintain` 的單一 AI JSON 階段會套用較短 timeout；候選題材萃取或細節擴寫 prompt 過大時會改用本地 evidence 規則與保守擴寫，並寫入 warning，避免定時題材庫維護卡在單一 MiniMax JSON 呼叫。
- `/topic_maintain` 的低階資料整理會限制實際送入 MiniMax 的段數；未送入低階模型的段落仍保留在本地題材 pipeline、來源索引與變更包稽核資料，不會被當成正式結論。
- `/topic_maintain` 的 Telegram 摘要與 review 文字會以繁體中文顯示「維護模式、審核狀態、信心度、變更動作」，避免把 `update`、`pending`、`medium`、`create_theme` 等內部值直接顯示給使用者。
...
- `/sector_strength` 補跑真實 M3 驗收後，當時的低階整理與高階模型 4 段分析、最終整合皆成功；最終整合 prompt 約 6 萬字元。
...
- 實測 `/topic_maintain --deep --model minimax` 已產生候選 23 筆、可審核變更 8 筆；仍需使用 `/topic_review` 檢視，再用 `/topic_confirm` 或 `/topic_reject` 處理。
- `prompt/base/base.md` 已補強主報告可讀性規則：不得原樣輸出 JSON key、snake_case、內部封包名稱、除錯欄位或模型內部參數名；若需要引用來源，使用 `[S001]` 這類來源編號。

## 共用服務層索引（2026-06-19）

系統目前以共用服務方向收斂：Command Runtime、Data Source Gateway、Cache / Artifact Registry、Backfill DAG、Entity Resolver、Data Gap / Report Quality、Structured Error / Health、System Health Snapshot、Prompt Bundle、Event Context、Feature Pack。新增指令、資料來源、prompt、回補項目或報告產物時，優先接入這些共用層，避免在單一指令內重複實作。完整架構與資料流請看 [docs/architecture.md](docs/architecture.md)。

2026-06-19 補強：`CommandRuntimeService` 已開始接手手動 `/backfill` 與定時回補的任務狀態、stop event、進度與失敗分類；`DataSourceGateway` 已接入價量 fallback policy；報告 JSON、Feature Pack 與 backfill complete marker 會登記到 Artifact Registry。

2026-06-19 補強：`CommandRuntimeService` 新增非破壞式任務鎖入口 `try_start_task()`、活動狀態查詢 `is_task_active()` 與集中 timeout 掃描 `mark_timeouts()`。新增背景任務時優先使用 runtime task_id 鎖，避免同一任務重複啟動後互相覆蓋狀態。

2026-06-19 補強：新增 `ScheduledTaskService`，定時任務的序列排隊、同類任務鎖、背景任務狀態、成功 / 失敗 / 略過訊息與錯誤分類會接到 `CommandRuntimeService`。啟動 CMD 的定時任務清單由 `SCHEDULED_JOB_REGISTRATIONS` 產生，不再使用容易過期的硬編碼摘要。
...
2026-06-19 補強：`ReportQualityService` 新增品質覆蓋清單，可查詢每個投研/新聞/雷達類指令需要的必要資料欄位；新增指令時需同步補 `required_fields_for_command()` 測試，避免資料缺口診斷只落在單一指令。

2026-06-19 補強：新增 `DataGapRefillService`（`research_center/data_gap_refill_service.py`）作為投研資料缺口補抓協調層。它接在 structured data、Feature Pack 與 data gap summary 之後，報告送 AI 前先嘗試用既有服務補缺口；補不到會保留 `missing_data_status` 與 `data_gap_refill.attempts` 原因，不會偽造資料，也不會中斷報告。

適用範圍：

- `/research`、`/research --deep`、`/research --source-only`：只補目標股票的 price/technical、chip、gross margin / financial、TDCC、news/events、topic/feature pack。
- `/value_scan`、`/value_scan --deep`：只補實際送入 AI 的 `ai_candidates`，不補全市場；單股模式只補該股票。
- `/theme`、`/theme_radar`：補題材 / topic context 與新聞上下文，不逐檔補全市場。
...
- 不改報告章節格式。
- 不改資料來源優先順序。
- 資料來源冷卻、quota 不足、timeout 或解析失敗時只記錄跳過 / 失敗原因。

2026-06-19 補強：`BackfillDagService` 新增節點事件模型，`run_full_backfill()` 會記錄主要節點 started/completed/skipped/failed 事件，complete marker 會保存 `backfill_dag_events` 與 `backfill_dag_event_summary`，用來回答「本次回補到底跑了哪些層」。

2026-06-19 補強：`DataSourceGateway` 新增來源健康與額度 snapshot，統一查詢來源冷卻、可用來源、FinMind 剩餘額度與 Fugle historical/intraday 剩餘額度。`SystemHealthService` 會直接取用這個共用 snapshot，避免各指令或狀態頁各自重寫來源健康摘要。

2026-06-19 補強：新增 `ResourceGuardService`，只限制背景資源池並觀測資源使用量。預設背景回補、背景 AI 維護與背景新聞各自最多 1 個並行任務；手動指令不納入限制，且不改 AI prompt、資料排序、評分權重、報告格式或來源優先順序。

2026-06-19 補強：新增 `tools/test_suite_manifest.py` 作為測試分層 manifest，固定 `fast_unit`、`integration`、`live_source`、`ai_smoke` 四層。`live_source` 與 `ai_smoke` 標記為 manual，避免一般回歸測試誤觸網路或 AI 額度。

## 2026-06-19 AI 指令參數健檢矩陣

`tools/ai_command_real_m3_validation.py` 已支援兩層健檢：

1. 真實 MiniMax M3 主流程驗收：
   `python tools/ai_command_real_m3_validation.py --include-parameter-matrix`
2. 只檢查參數解析與覆蓋，不呼叫 AI：
   `python tools/ai_command_real_m3_validation.py --parameter-matrix-only`

參數矩陣會輸出：

- `parameter_matrix.json`
- `parameter_matrix.md`
...
矩陣同時檢查不支援參數是否會被拒絕，例如 `/research --top`、`/news refresh --deep`、`/radar --brief`。

真實驗收報告仍會保存每個指令的 raw command、prompt 路徑、來源數、prompt 字元數、輸出報告、品質檢查與 runtime issue。參數矩陣用來避免只跑 10 個代表指令，卻漏掉日期、輸出格式、來源、天數、Radar AI 短評等參數行為。
## 2026-06-19 AI 指令真實健檢補強

本次全 AI 指令健檢以 MiniMax M3 真實執行 10 個入口：`/research`、`/value_scan`、`/macro`、`/theme`、`/theme_flow`、`/theme_radar`、`/sector_strength`、`/radar`、`/news refresh`、`/topic_maintain`。

修正重點：

- `/radar`：健檢工具會統計低階整理與每段 AI 短評的 `prompt=... chars`，不再顯示 prompt 字數為 0。
- `/theme_flow`：報告提示詞強制輸出「資金流向 / 資金輪動」章節；若資料不足，必須明確說明缺口，不可省略。
- `/topic_maintain`：題材細節補全必須以繁體中文填寫風險、反證、資料缺口與後續驗證方向。
- `/theme_radar`、`/sector_strength`、`/theme_flow`：若高階模型核心入模包單段過大，系統會依 JSON 結構切段；此動作只切段、不刪資料，完整資料仍保留於 JSON、來源檔與 HTML 附錄。
- `/theme_radar`、`/sector_strength`：分段策略以較少但仍受控的 11～16 萬字元資料段執行，最後整合 prompt 改用短來源索引、短分段摘錄與精簡本地摘要，避免整合階段再次膨脹。完整分段底稿與完整來源仍保留於 prompt log、JSON 與 HTML 附錄。
- `/topic_maintain`、`/radar`：健檢工具只以 `progress.log` 統計實際送入模型的 `prompt=... chars`，避免 `worker_stdout.log` 重複加總；`original_prompt=... chars` 只代表估算基準，不列入實際模型消耗。
- 健檢報告會保存每個指令的來源數、prompt 字數、粗估 token、輸出路徑、品質檢查與 runtime issue。

目標是維持報告完整性、可信度、精準度與想像力，同時避免單一 prompt 過大造成 MiniMax M3 產出過慢或不穩。

## 2026-06-20 AI 搜尋品質與大型題材雷達入模補強

- 搜尋 fallback 不只看來源數量。`/research`、`/news refresh`、`/theme`、`/theme_flow` 等流程會同時檢查官方來源、日期新鮮度、來源層級與題材相關性；即使來源數已達標，只要品質不足仍會啟用補搜或 fallback。
- `/theme_flow` 已和 `/theme` 使用同一套題材相關性品質檢查，避免來源數很多但內容與指定題材不相關時過早停止搜尋。
- 報告與 TG 顯示層會把常見英文指標欄位轉成繁體中文，例如 `change pct` 轉為「漲跌幅」、`sector score` 轉為「族群分數」、`sector state` 轉為「族群狀態」、`theme hit count` 轉為「題材命中數」。
- `/theme_radar` 的高階入模 `stock_index` 會保留股票、題材、供應鏈角色、關係強度、來源層級、非零風險/反證/缺口數；零值統計不再反覆入模，過長受惠邏輯會保留短句，完整原始資料仍保留於 JSON / HTML 附錄。
- 使用 2026-06-20 真實 `/theme_radar` 報告資料重建 high model package 驗證：整包約由 461k chars 降至 232k chars，`stock_index` 約由 223k chars 降至 66k chars，屬於去重與索引化，不是刪除核心資料。

## 2026-06-21 AI 健檢補強：分段上限、可恢復錯誤與 Prompt 清洗

- `/theme_radar` 分段分析超過上限時，現在會把相鄰分段再次合併到最多 8 段；合併只改變入模結構，不刪除任何 payload，完整資料仍保留在報告 JSON、來源檔與 HTML 附錄。
- 分段 prompt 與 fallback prompt 的內文改為可讀繁體中文，避免高階模型或使用者報告看到亂碼、內部欄位或不可理解說明。
- 分段 prompt 送出前會移除 Unicode 私用區與替代字元，避免 MiniMax M3 因壞字元觸發 `cp950` 編碼錯誤；這只移除損壞字元，不移除股票、題材、數據或來源內容。
- `/topic_maintain` 更新模式的 AI 細節擴寫限制為候選抽取加 2 批細節擴寫；初始化模式最多 4 批。超出批次改用本地保守擴寫並寫入警告，避免定時題材庫維護長時間卡住。
- 健檢工具現在會把「MiniMax 分類 timeout 後輕量 retry 成功」與「低階整理失敗後精簡重試成功」視為已恢復事件，不再誤判整個指令失敗；未恢復的失敗仍會列為 runtime issue。
- 健檢工具會解析 `/theme_radar`、`/theme_flow`、`/sector_strength` 的分段 prompt 字元數，讓 prompt/token 成本監控不再漏掉分段流程。
- `summary.md` 只有在指令失敗、品質檢查未通過或仍有 runtime issue 時，才列入「未達標或需檢查項目」，避免把已通過的正向檢查誤列為問題。

### 2026-06-22 健檢重算與分段判斷補強

- 健檢工具會同時記錄累計 `prompt_chars` 與單次最大 `max_prompt_chars`。分段指令的「Prompt 未失控」以單次最大呼叫判斷，累計 prompt 只作為成本觀察，避免把安全分段後的總量誤判為單次爆量。
- 若 MiniMax 低階分段曾失敗但後續「精簡重試成功」且最終 `failed=0`，健檢會視為已恢復事件，不再把整個 `/value_scan` 或 `/theme_radar` 標成低階模型失敗。
- 重新掃描既有健檢紀錄時，會先清除舊的 runtime issue 再重新計分，避免修正後仍殘留過期問題標籤。
- `/topic_maintain` 品質探針不再把原始 JSON 直接混入使用者輸出檢查；原始更新包仍保存於 JSON，健檢只檢查可讀的風險、反證、資料缺口與後續驗證摘要，避免把內部欄位名稱誤判為報告外露。

## 2026-06-21 全 AI 指令 MiniMax M3 真實驗證

- 已用 `tools/ai_command_real_m3_validation.py --include-parameter-matrix` 重新跑完整 10 個 AI 指令：`/research`、`/value_scan`、`/macro`、`/theme`、`/theme_flow`、`/theme_radar`、`/sector_strength`、`/radar`、`/news refresh`、`/topic_maintain`。
- 驗證使用實際 MiniMax M3、實際搜尋與 WebFetch 流程，不是 mock。10 個指令全部成功，驗證摘要位於 `logs/ai_command_real_m3_validation/20260621_full_after_final_fixes/summary.md`。
- 參數矩陣 21/21 通過，涵蓋 `--model`、`--deep`、`--date`、`--top`、`--days`、`--source`、`--source-only`、`--score`、`--brief`、`--from-radar`、`--ai-top`、`--no-ai-comment`、`--no-html`、`--no-md`、`--no-json` 等支援或拒絕行為。
- 已追加參數實際執行 smoke：12/12 成功，涵蓋 `--source-only`、`--score`、`--date`、`--top`、`--days`、`--source`、`--from-radar`、`--no-ai-comment`、`--no-html`、`--no-md`、`--no-json` 等常用參數的真實流程；摘要位於 `logs/ai_command_real_m3_validation/20260621_parameter_execution_smoke/summary.md`。
- `/theme_radar` 真實驗證中原始 prompt 約 495 萬字元，經保真去重與受控分段後成功產出；實際分段 7 段，未再出現 80 段以上的失控分段。
- `/sector_strength` 與 `/theme_flow` 也走受控分段；分段結果會保存 prompt 字元數、估算 token、來源數與耗時。
- `/radar` 定時推播長訊息已驗證分段發送；測試覆蓋推播過長與單段發送失敗後的處理。
- 驗證工具修正 `summary.md` 與 `quality_review.json` 不一致問題；摘要會以最新品質檢查結果為準，避免實際通過卻在 summary 誤報未達標。
- 完整測試通過：`python -m unittest discover tests`，共 1205 tests OK。

## 2026-06-21 選股共用硬篩選條件

- 基礎硬篩條件集中由 `candidate_filter_service.py` 管理。
- 既有 `/scan`、精選、技術與籌碼策略仍使用原硬篩；老蕭策略只共用相容的最低股價與 20 日均量設定，刻意不套最高股價與最低月營收硬門檻。
- 目前預設門檻：股價 10 到 500、20 日均量至少 500、最近月營收至少 `40000000`（四千萬）。
- 各策略自己的後續判斷不納入共用硬篩，例如毛利率分級、技術指標、籌碼策略、Radar 評分與價值重估分數。
## 2026-06-22 Radar 快取與效能補強

- Radar 結果快取 `.cache/radar_results.json` 改為輕量索引：只保存 radar_id、日期、來源、候選摘要與 artifact 路徑。
- 完整候選股、evidence pack、AI 分析與來源仍保存在 `reports/radar/YYYY-MM-DD/radar_xxx/`，不刪除完整資料。
- 若偵測到舊版 Radar cache 過大，系統會從 `reports/radar` artifacts 自動重建索引，避免每次讀取巨大 JSON 造成 `/radar_more`、定時 Radar 或回測卡住。
- Radar 籌碼候選池與籌碼評級補強共用同一份 5 個交易日輕量籌碼 grade maps，避免同一輪 Radar 重複抓取籌碼資料。
- 本次調整目標是提升穩定性與效率；完整資料仍在 report artifacts，可供 HTML / JSON / 稽核流程追溯。

## 2026-06-22 /research fallback 狀態標示

- `/research` 產出檔案成功不等於正式 AI 分析成功。
- 正式 AI 完成時，metadata 會標示 `ai_status=ai_success`。
- AI 失敗但本地 fallback 成功時，結果狀態會是 `fallback_success`，metadata 會標示 `ai_status=fallback_success`。
- 分段分析若段落成功但最後高階模型整合失敗，仍視為 fallback，不會標成正式 AI 成功。
- fallback Markdown / HTML 會明確顯示「這不是正式 AI 完成報告，而是本地資料 fallback 報告」。
## 2026-06-22 AI 健檢狀態補強

- `/news refresh` 現在會區分「AI 分類完整成功」與「AI 分類 timeout 後改用本地規則」。
- 若新聞分類進入本地規則 fallback，回傳 metadata 會標示 `classification_status=partial_success`、`local_fallback_count` 與 `ai_classified_count`。
- `tools/ai_command_real_m3_validation.py` 讀到新聞分類 fallback 或 MiniMax API runtime issue 時，不再把該輪視為完整 `ai_success`，而會標成 `partial_success`，健檢也不算完整通過。
- `/theme_flow` prompt 新增資金流硬性要求：必須說明資金流向、族群輪動、量價強弱、法人籌碼；資料不足時也要明確寫出「目前資金流證據不足」。

## 2026-06-22 搜尋來源冷卻補強

- Gemini Search fallback 現在會接入 `SourceHealthManager`。
- 若 Gemini Search discovery 遇到 429、quota、rate limit、403、blocked 或憑證錯誤，會立即進入短期 cooldown，當輪停止剩餘 fallback 任務。
- cooldown 期間會在 `gemini_search_discovery.reason=source_cooldown` 中記錄原因，不再反覆對同一個不可用來源送出大量失敗請求。
- 這項補強目標是降低 `/research`、`/news refresh`、`/theme`、`/macro` 等共用搜尋流程的無效等待與 timeout 風險。

## 2026-06-23 題材庫維護 M3 穩定性補強

- 低階資料整理與題材庫維護的 MiniMax 預設已統一為 MiniMax M3；`minimax_low_model` 也預設使用 MiniMax M3。
- `/topic_maintain` 的候選題材萃取 prompt 進一步降載，避免單次候選抽取卡在過大的 JSON prompt。
- 候選題材萃取若遇到 MiniMax timeout，會用更小的候選資料包自動 retry 一次；retry 成功時不視為 fallback。
