...
- /scan
  顯示互動式策略選單，可選擇財報營收選股、四套籌碼策略、技術面選股、精選選股或全部執行。
- /scan 日期
...
- 第 7 項「全部執行」會依序產生財報營收、四套籌碼、技術面與精選選股報告；精選選股會放在最後輸出，作為交叉命中總結。
- 「全部執行」完成後會保存一筆最近掃描紀錄到 `.cache/recent_scan_results.json`，方便後續檢查或 `/value_scan` 使用最近掃描結果。
...
- `/scan` 選單中的「全部執行」也會在財報、籌碼與技術面報告後執行精選選股，並將全部執行的合併結果保存為最近掃描紀錄。
...
- `/scan` 產生精選選股報告後，會保存最近掃描結果到 `.cache/recent_scan_results.json`；若執行「全部執行」，也會保存一筆包含財報、籌碼、技術面與精選選股摘要的合併紀錄。
- 保存內容包含：掃描類型、日期、建立時間、股票代號清單、候選數與摘要。
- 股票代號清單會先從報告中的股票列格式抽取，再用 `stock_list.json` 驗證；日期年份、掃描統計數字或其他 4 位數字不會被當成股票代碼，真正出現在股票列且存在於股票清單的 4 位代碼仍會保留。
