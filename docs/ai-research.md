...
所有報告型 AI 指令會載入 `prompt/rules/output_quality_rules.md`。這份規則負責穩定輸出品質，不取代各指令的主模板。

核心要求：

1. 重要結論必須歸類為 `verified_fact`、`reasoned_inference`、`market_hypothesis`、`sentiment_signal` 或 `insufficient`。
2. 主要章節要回答：確認事實、市場解讀、爆發條件、缺少證據、失敗條件、分類標籤。
3. 指令要求 JSON 時，只輸出可解析 JSON。
4. 指令要求 Markdown 報告時，章節、表格與結論格式要穩定。
5. 中間整理 prompt 只做資料整理，不做完整投研判斷。

source-only 模式仍可使用品質與來源可信度規則，但不載入市場想像力規則。

## Prompt 維護位置

| 類型 | 檔案 |
|---|---|
| 基礎行為 | `prompt/base/base.md` |
| 輸出品質 | `prompt/rules/output_quality_rules.md` |
| 來源可信度 | `prompt/rules/source_quality_rules.md` |
| 市場想像力 | `prompt/rules/embedded_market_imagination_rules.md` |
| 搜尋整理 | `prompt/discovery/discovery_task.md` |
| 低階資料整理 | `prompt/workflow/low_model_digest.md`、`prompt/workflow/low_model_digest_retry.md` |
| Radar 短評 | `prompt/radar/radar_ai_comment.md` |
| 新聞分類 | `prompt/news/news_summary.md` |
| 題材維護 | `prompt/topic/topic_maintain.md` |
## 報告推演骨架

為避免 AI 報告只變成公開資訊整理，主要報告模板與本地 fallback 報告都必須保留以下推演骨架：
...
這些段落會嵌入既有報告與評分架構，不取代財務硬指標、題材軟指標、飆股基因、價值重估或最終研究優先度。若只有市場故事但缺少營收、產品、客戶、供應鏈、籌碼或來源證據，必須標示為 `market_hypothesis`、`sentiment_signal` 或 `insufficient`。

`/value_scan` 的本地 fallback 也會產出同樣骨架，避免 AI 模型失敗時報告只剩結構化 JSON。逐檔附加分析會補上市場故事、早期線索、催化劑、缺少訊號、失敗條件與想像力結論。

## 報告推演品質檢查

`tools/ai_report_coverage_check.py` 除了檢查 `ai_workflow_coverage`，也會讀取報告 JSON 或同名 Markdown，檢查是否具備推演骨架。輸出欄位包含：

- `narrative_quality_status`
- `narrative_missing_sections`
