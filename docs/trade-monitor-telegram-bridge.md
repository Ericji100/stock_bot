...
`trade_monitor_bridge.py` 將 Codex 台指期監控產生的同一份 canonical message 轉為 Telegram entities、分段並交給既有 Bot 發送。Bridge 不分析或改寫盤勢，也不 import、啟動或輪詢 `main.py`。
...
Get-Content -LiteralPath .\canonical-message.txt -Raw -Encoding UTF8 |
  python -m trade_monitor_bridge --event-id <SHA256_EVENT_ID> --decision NOTIFY
```
...
completed = subprocess.run(
    [python_executable, "-m", "trade_monitor_bridge", "--event-id", event_id, "--decision", decision],
...
```powershell
Get-Content -LiteralPath .\canonical-message.txt -Raw -Encoding UTF8 |
  python -m trade_monitor_bridge --event-id <SHA256_EVENT_ID> --decision NOTIFY --dry-run
```
...
程式亦提供 `trade_monitor_bridge.generate_event_id(...)` 供監控端使用。同一個成功送達的 event ID 再次執行時會回傳 `duplicate`，不重新推送；不同 event ID 可正常發送。
...
`trade_monitor_resume_guard.py` 只追蹤監控執行連續性，不參與盤勢分析。每輪分析開始前執行：

```powershell
python -m trade_monitor_resume_guard begin --gap-seconds 180
```
...
```powershell
python -m trade_monitor_resume_guard complete --run-id <RUN_ID> --resume-delivered true
```
