...
1. 公司在題材中的角色，例如設計、製造、封裝、材料、設備、系統整合、通路或終端應用。
2. 上游 / 中游 / 下游位置，不可只寫「受惠」或「概念股」。
3. 產品、客戶或應用場景；無法確認時必須寫入 `missing_data`。
4. 營收曝險；不可捏造百分比，無資料時使用 `unknown` 並標示待查證。
5. 支撐證據與反證；至少保留來源名稱、source_level、內容摘要與 URL（若有）。

請區分：

- `long_term_theme`：有產業趨勢、供應鏈角色、產品或營收邏輯支撐，可進入題材庫。
- `short_term_news`：單一新聞、短線事件或一次性題材，只能作觀察，不可直接升級為長期題材。
- `theme_riding`：公司只是被市場歸類或蹭題材，缺少產品、客戶、營收或公告證據時，必須降為 candidate 或寫入 missing_data。

不得只靠單篇新聞寫入長期題材庫；至少需要供應鏈邏輯、公司角色與可追溯證據。若資料不足，保留候選但降低 confidence。
