"""Build FINAL Candidate 2 anonymous, causal, atomic-question packets.
...
PACKET_VERSION = "hybrid-v3-atomic-question-packet-v3"
BUILDER_VERSION = "hybrid-v3-atomic-packets-v3"
BUILDER_STATUS = "FINAL"
FORMAL_REVIEW_POINT_POLICY = "CANDIDATE2_EVENT_DRIVEN_CAUSAL_REVIEW_BOUNDARIES"
...
HYPOTHESIS_ENUMERATION_POLICY = "CANDIDATE2_SCENARIO_TYPED_CURRENT_TRIGGER_RELATIONS"
...
def load_and_validate_sampling_contract(path: Path | None = None) -> dict[str, Any]:
    """Read the versioned Candidate2 contract and cross-check code constants."""
    contract_path = Path(path or DEFAULT_SAMPLING_CONTRACT)
    contract = _read_json(contract_path)
    if contract.get("contract_version") != SAMPLING_CONTRACT_VERSION:
        raise FormalBuildIntegrityError("unexpected Candidate2 sampling contract version")
    if contract.get("status") not in {"DRAFT_NOT_FINAL", "FINAL"}:
        raise FormalBuildIntegrityError("Candidate2 sampling contract has an invalid status")
    if tuple(contract.get("strata_order") or ()) != SAMPLING_STRATA_CANONICAL_ORDER:
        raise FormalBuildIntegrityError("Candidate2 sampling strata differ from builder constants")
    quotas = contract.get("quotas_per_block")
    if not isinstance(quotas, dict) or tuple(quotas) != SAMPLING_STRATA_CANONICAL_ORDER:
        raise FormalBuildIntegrityError("Candidate2 sampling quotas do not have canonical keys")
    if quotas != FORMAL_SAMPLING_QUOTAS:
        raise FormalBuildIntegrityError("Candidate2 sampling quotas differ from builder constants")
    if contract.get("reserve_block_count") != FORMAL_RESERVE_BLOCK_COUNT:
        raise FormalBuildIntegrityError("Candidate2 reserve count differs from builder constants")
    if tuple(contract.get("allowed_wait_boundary_reasons") or ()) != WAIT_BOUNDARY_REASON_ENUM:
        raise FormalBuildIntegrityError("Candidate2 WAIT reasons differ from builder constants")
    if tuple(contract.get("required_source_row_fields") or ()) != SOURCE_ROW_FIELDS:
        raise FormalBuildIntegrityError("Candidate2 source-row fields differ from builder contract")
    if tuple(contract.get("packet_sampling_fields_forbidden") or ()) != PACKET_SAMPLING_FIELDS_FORBIDDEN:
        raise FormalBuildIntegrityError("Candidate2 packet sampling-field guard differs from builder contract")
    return contract
...
def _relation_candidates(
    ...
    # Fresh Q1 is mutually exclusive with a valid parent/correction copy.
    # It still needs a completed early UP anchor, but the current relation is
    # the as-of UP attack rather than a recycled Macro relation.
    if up_trigger_today and not parent_correction_pairs:
        ...
        if fresh_anchors:
            ...
            values = {
                ...
                "relation_state": "CURRENT_FRESH_UP_WITHOUT_VALID_PARENT_COPY",
                "enumeration_policy": HYPOTHESIS_ENUMERATION_POLICY,
            }
            append_relation(values)
    ...
    *,
    ...
    for state in points:
        ...
        review_id = "D-" + hmac.new(
            identity_salt,
            f"{private_code}|{day}|CANDIDATE2_V3".encode(),
            hashlib.sha256,
        ).hexdigest()[:24]
        packets.append(
            build_atomic_packet(
    ...
    *,
    ...
    return {
        ...
        **sampling_contract_pin,
        ...
        "expected_candidate2_review_points": expected_review_points,
