"""Build FINAL Candidate 3 anonymous, causal, atomic-question packets.
...
try:
    ...
    from .hybrid_v3_objective_state_v3 import (
...
DEFAULT_SAMPLING_CONTRACT = ROOT / "config/hybrid_multilabel_sampling_v3.json"
DEFAULT_FREEZE_SOURCE_MANIFEST = DAILY / "packet_manifest.json"

PACKET_VERSION = "hybrid-v3-atomic-question-packet-v4"
BUILDER_VERSION = "hybrid-v3-atomic-packets-v4"
BUILDER_STATUS = "FINAL"
FORMAL_REVIEW_POINT_POLICY = "CANDIDATE3_EVENT_DRIVEN_CAUSAL_REVIEW_BOUNDARIES"
...
HYPOTHESIS_ENUMERATION_POLICY = "CANDIDATE3_MULTI_ROUTE_CURRENT_TRIGGER_CHALLENGES"
...
PRIMARY_SAMPLING_FOCUS_PRIORITY = (
    "MACRO_DEFENSE_REMOVE_PROXY",
    "WAIT_POLICY_BOUNDARY",
    "BEAR_REVERSAL_OBJECTIVE_PROXY",
    "FRESH_Q1_OBJECTIVE_PROXY",
...
PACKET_SAMPLING_FIELDS_FORBIDDEN = (
    "eligible_sampling_strata",
    "primary_sampling_focus",
    "sampling_stratum",
    "sampling_focus",
...
def load_and_validate_sampling_contract(path: Path | None = None) -> dict[str, Any]:
    """Read the shared versioned sampling contract and cross-check constants."""
    contract_path = Path(path or DEFAULT_SAMPLING_CONTRACT)
    contract = _read_json(contract_path)
    if contract.get("contract_version") != SAMPLING_CONTRACT_VERSION:
        raise FormalBuildIntegrityError("unexpected sampling contract version")
    if contract.get("status") not in {"DRAFT_NOT_FINAL", "FINAL"}:
        raise FormalBuildIntegrityError("sampling contract has an invalid status")
    if tuple(contract.get("strata_order") or ()) != SAMPLING_STRATA_CANONICAL_ORDER:
        raise FormalBuildIntegrityError("sampling strata differ from builder constants")
    quotas = contract.get("quotas_per_block")
    if not isinstance(quotas, dict) or tuple(quotas) != SAMPLING_STRATA_CANONICAL_ORDER:
        raise FormalBuildIntegrityError("sampling quotas do not have canonical keys")
    if quotas != FORMAL_SAMPLING_QUOTAS:
        raise FormalBuildIntegrityError("sampling quotas differ from builder constants")
    if contract.get("reserve_block_count") != FORMAL_RESERVE_BLOCK_COUNT:
        raise FormalBuildIntegrityError("sampling reserve count differs from builder constants")
    if tuple(contract.get("allowed_wait_boundary_reasons") or ()) != WAIT_BOUNDARY_REASON_ENUM:
        raise FormalBuildIntegrityError("sampling WAIT reasons differ from builder constants")
    if tuple(contract.get("required_source_row_fields") or ()) != SOURCE_ROW_FIELDS:
        raise FormalBuildIntegrityError("sampling source-row fields differ from builder contract")
    if tuple(contract.get("packet_sampling_fields_forbidden") or ()) != PACKET_SAMPLING_FIELDS_FORBIDDEN:
        raise FormalBuildIntegrityError("packet sampling-field guard differs from builder contract")
    return contract


def _sampling_contract_pin(path: Path | None = None) -> dict[str, Any]:
    contract_path = Path(path or DEFAULT_SAMPLING_CONTRACT).resolve()
    contract = load_and_validate_sampling_contract(contract_path)
    return {
        "sampling_contract_version": str(contract["contract_version"]),
        "sampling_contract_status": str(contract["status"]),
...
def build_daily_objective_states(
    ...
    *,
    ...
    for monitored_ordinal, (index, row) in enumerate(monitored.iterrows()):
        ...
        initial_selection_frontier = bool(monitored_ordinal == 0 and watchlist_active)
        reselection_frontier = bool(reselected_today)
        up_attack_without_stop_frontier = bool(
            watchlist_active
            and len(prefix) >= INDICATOR_WARMUP_BARS
            and new_up_attack
            and not causal_small_bull_stop
        )
        lazy_reasons = [
            reason
            for condition, reason in (
                ...
                (initial_selection_frontier, "INITIAL_SELECTION_BOUNDARY"),
                (reselection_frontier, "RESELECTION_BOUNDARY"),
                (up_attack_without_stop_frontier, "UP_ATTACK_WITHOUT_CAUSAL_STOP"),
            )
            if condition
        ]
...
def _relation_candidates(
    ...
    """Build scenario-typed relations tied to the current causal UP event.

    Candidate 1 treated any historical DOWN relation as a Bear subject and
    emitted identical Macro/Fresh relation sets.  Candidate 2 admits a route
    only when the relation explicitly reaches the current ``as_of`` trigger.
    """

    rows: list[dict[str, Any]] = []
    attacks_today = sorted(
        (
            attack for attack in objective.get("attacks") or []
            if attack["direction"] == "UP" and attack["confirmed_on"] == as_of
        ),
        key=lambda row: (str(row["scale"]), str(row["attack_id"])),
    )
    small_up_today = [attack for attack in attacks_today if attack["scale"] == "SMALL"]
    up_trigger_today = bool(attacks_today)
    streams: dict[tuple[str, str], list[dict[str, Any]]] = {}
    for anchor in anchors:
        streams.setdefault((anchor["hypothesis_type"], anchor["scale"]), []).append(anchor)

    def append_relation(values: dict[str, Any]) -> None:
        candidate = {
            "candidate_id": "RELATION_CANDIDATE:R-" + canonical_sha256(values)[:16],
            **values,
        }
        if candidate["candidate_id"] not in {row["candidate_id"] for row in rows}:
            rows.append(candidate)

    # Macro and mature relations require a completed UP parent followed by a
    # completed DOWN correction.  The current UP leg is the attack confirmed
    # on this exact review day; it is intentionally not treated as a completed
    # pivot leg.
    parent_correction_pairs: list[tuple[dict[str, Any], dict[str, Any]]] = []
    for stream in streams.values():
        stream = sorted(
            stream, key=lambda row: (str(row["available_on"]), str(row["candidate_id"]))
        )
        for parent, correction in zip(stream, stream[1:]):
            if not (
                parent["direction_hint"] == "UP"
                and correction["direction_hint"] == "DOWN"
                and str(correction["available_on"]) <= as_of
            ):
                continue
            parent_correction_pairs.append((parent, correction))
            if not up_trigger_today:
                continue
            current_number = int(parent["same_direction_attack_number"]) + 1
            prior_copy_count = max(0, current_number - 2)
            scenario = (
                "MACRO_COPY_RESONANCE"
                if prior_copy_count == 0
                else "MATURE_TREND_PULLBACK"
            )
            values = {
                ...
                "relation_state": "COMPLETED_UP_PARENT_DOWN_CORRECTION_CURRENT_UP_TRIGGER",
                "enumeration_policy": HYPOTHESIS_ENUMERATION_POLICY,
            }
            append_relation(values)

    # Scenario exclusivity is a semantic verdict, not an objective-enumeration
    # shortcut. A remote or invalidated historical parent/correction pair must
    # not permanently suppress a later Fresh-Q1 challenge. Enumerate the
    # current fresh-anchor hypothesis alongside competing Macro/Mature subjects;
    # REL_CURRENT_LEG_IS_FRESH_ANCHOR and the reducer decide whether it is fresh.
    if up_trigger_today:
        fresh_anchors = sorted(
            (
                anchor for anchor in anchors
                if anchor["direction_hint"] == "UP"
                and int(anchor["same_direction_attack_number"]) == 1
            ),
            key=lambda row: (str(row["available_on"]), str(row["candidate_id"])),
        )
        if fresh_anchors:
            current = fresh_anchors[-1]
            values = {
                ...
                "relation_state": "CURRENT_FRESH_UP_CANDIDATE_REQUIRES_SEMANTIC_EXCLUSIVITY",
                "enumeration_policy": HYPOTHESIS_ENUMERATION_POLICY,
            }
            append_relation(values)

    # Bear reversal is a distinct DOWN-parent -> current SMALL-UP relation.
    # LR requires an active boundary; RR may reference the causally breached
    # boundary that created the right-side state.
    left_right = objective.get("left_right") or {}
    phase = str(left_right.get("phase") or "NONE")
    large_dow = str((objective.get("dow", {}).get("large") or {}).get("dow_state") or "UNDEFINED")
    large_control = str((objective.get("controls", {}).get("large") or {}).get("state") or "UNRESOLVED")
    boundary_ref = str(left_right.get("boundary_defense_ref") or "")
    large_bear_defenses = {
        str(row["defense_id"]): row
        for row in objective.get("defenses") or []
        if row["scale"] == "LARGE" and row["side"] == "BEARISH"
    }
    boundary = large_bear_defenses.get(boundary_ref)
    boundary_valid = bool(
        boundary
        and (
            (phase == "LR" and boundary.get("status") == "ACTIVE")
            or (phase == "RR" and boundary.get("status") == "BREACHED")
        )
    )
    bear_context = bool(
        small_up_today
        and phase in {"LR", "RR"}
        and (large_dow == "BEAR" or large_control == "DOWN_CONTROL")
        and boundary_valid
    )
    if bear_context:
        down_parents = sorted(
            (
                anchor for anchor in anchors
                if anchor["direction_hint"] == "DOWN" and anchor["scale"] == "LARGE"
            ),
            key=lambda row: (str(row["available_on"]), str(row["candidate_id"])),
        )[-2:]
        for parent in down_parents:
            for current_attack in small_up_today[-1:]:
                values = {
                    "relation_type": "BEAR_DOWN_PARENT_TO_CURRENT_SMALL_UP",
                    "eligible_scenarios": ["BEAR_REVERSAL_LEFT_RIGHT"],
                    "parent_anchor_ref": parent["candidate_id"],
                    "correction_anchor_ref": None,
                    "current_leg_ref": "CURRENT_CONTEXT",
                    "current_attack_ref": current_attack["attack_id"],
                    "parent_scale": "LARGE",
                    "correction_scale": None,
                    "current_scale": "SMALL",
                    "parent_available_on": parent["available_on"],
                    ...
                    "current_available_on": as_of,
                    "available_on": as_of,
                    "parent_direction": "DOWN",
                    "correction_direction": "NONE",
                    "current_direction": "UP",
                    "campaign_id": parent["campaign_id"],
                    "large_bear_defense_ref": boundary_ref,
                    "large_bear_defense_status": boundary["status"],
                    "left_right_phase": phase,
                    "taiji_generation": "REVERSAL_PROBE",
                    "same_direction_attack_number": 1,
                    "completed_prior_copy_count": 0,
                    "relation_state": "CURRENT_SMALL_UP_BOUND_TO_BEAR_PHASE",
                    "enumeration_policy": HYPOTHESIS_ENUMERATION_POLICY,
                }
                append_relation(values)
        ...
        rows.append(
            {
                ...
                "relation_type": "NO_CAUSAL_ROUTE_RELATION",
                "eligible_scenarios": [],
                "parent_anchor_ref": None,
                "correction_anchor_ref": None,
                "current_leg_ref": "CURRENT_CONTEXT",
                "current_attack_ref": None,
                "parent_scale": None,
                "correction_scale": None,
                "current_scale": None,
                "parent_available_on": None,
    ...
    ordered = sorted(
        rows, key=lambda row: (str(row.get("available_on") or ""), str(row["candidate_id"]))
    )
...
def _scenario_hypotheses(
    ...
    *,
    ...
    """Enumerate only relations that were objectively typed for each route.

    The relation builder is the single causal authority for route reachability.
    This layer links its subject references to causal stop candidates without
    broadening a typed relation back into another scenario.
    """
    ...
    for scenario in SCENARIOS:
        for relation in relations:
            eligible_scenarios = set(relation.get("eligible_scenarios") or [])
            if scenario not in eligible_scenarios:
                continue
            if (
...
def _objective_facts(
    *,
    ...
    large_control = str(objective["controls"]["large"]["state"])
    ...
    material_signatures = {
        ...
        for scenario, route_hypotheses in hypotheses.items()
        if data_sufficiency_by_route[scenario]["status"] is True
        for hypothesis in route_hypotheses
    }
    material_conflict = len(material_signatures) > 1
    wait_reason_set: set[str] = set()
    if "INITIAL_SELECTION" in events:
        wait_reason_set.add("INITIAL_SELECTION_BOUNDARY")
    if "RESELECTION_AFTER_REMOVAL" in events:
        wait_reason_set.add("RESELECTION_BOUNDARY")
    if trigger and not active_small_bull_stops:
        wait_reason_set.add("UP_ATTACK_WITHOUT_CAUSAL_STOP")
    if trigger and not any(
        route["status"] is True for route in data_sufficiency_by_route.values()
    ):
        wait_reason_set.add("UP_ATTACK_WITH_INSUFFICIENT_ROUTE_OBJECTIVE_EVIDENCE")
    if material_conflict:
        wait_reason_set.add("MATERIAL_OBJECTIVE_HYPOTHESIS_CONFLICT")
    wait_boundary_reasons = [
        reason for reason in WAIT_BOUNDARY_REASON_ENUM if reason in wait_reason_set
    ]
    ...
    return {
        ...
        "large_control_state": large_control,
        "left_right_phase": left_right,
        "bear_reversal_context": bool(
            (large_dow == "BEAR" or large_control == "DOWN_CONTROL")
            and left_right in {"LR", "RR"}
            and large_bear is not None
        ),
        ...
        "material_objective_hypothesis_conflict": material_conflict,
        "material_objective_signature_count": len(material_signatures),
        "wait_boundary_reasons": wait_boundary_reasons,
...
def build_stock_packets(
    *,
    ...
    for state in points:
        day = str(state["as_of"])
        review_id = "D-" + hmac.new(
            identity_salt,
            f"{private_code}|{day}|CANDIDATE3_V4".encode(),
            hashlib.sha256,
        ).hexdigest()[:24]
        packets.append(
            build_atomic_packet(
...
def prepare_formal_build_plan(
    *,
    ...
    sampling_contract_pin = _sampling_contract_pin()
    ...
    plan = {
        ...
        "objective_engine_code_sha256": file_sha256(ROOT / "scripts/hybrid_v3_objective_state_v3.py"),
        "pivot_scale_disclosure": PIVOT_SCALE_DISCLOSURE,
...
def eligible_sampling_strata(packet: dict[str, Any]) -> list[str]:
    """Return every outcome-blind stratum supported by objective packet facts."""
    facts = packet.get("objective_facts") or {}
    hypotheses = facts.get("scenario_hypotheses") or {}
    sufficiency = facts.get("data_sufficiency_by_route") or {}
    ...
    for stratum, scenario in scenario_for_stratum.items():
        route = sufficiency.get(scenario) or {}
        if hypotheses.get(scenario) and route.get("status") is True:
    ...
    wait_reasons = facts.get("wait_boundary_reasons") or []
    if not isinstance(wait_reasons, list) or len(wait_reasons) != len(set(wait_reasons)):
        raise FormalBuildIntegrityError("wait_boundary_reasons must be a unique list")
    if any(reason not in WAIT_BOUNDARY_REASON_ENUM for reason in wait_reasons):
        raise FormalBuildIntegrityError("wait_boundary_reasons contains an unknown reason")
    if wait_reasons != [reason for reason in WAIT_BOUNDARY_REASON_ENUM if reason in set(wait_reasons)]:
        raise FormalBuildIntegrityError("wait_boundary_reasons is not in canonical order")
    if wait_reasons:
        eligible.add("WAIT_POLICY_BOUNDARY")
    ordered = [stratum for stratum in SAMPLING_STRATA_CANONICAL_ORDER if stratum in eligible]
    if not ordered:
        raise FormalBuildIntegrityError("review point has no objective sampling eligibility")
    return ordered


def primary_sampling_focus(eligible: Sequence[str]) -> str:
    """Choose a diagnostic focus only; it never removes other eligibility."""
    values = list(eligible)
    if values != [value for value in SAMPLING_STRATA_CANONICAL_ORDER if value in set(values)]:
        raise FormalBuildIntegrityError("eligible sampling strata is not canonical")
    for stratum in PRIMARY_SAMPLING_FOCUS_PRIORITY:
        if stratum in values:
            return stratum
    raise FormalBuildIntegrityError("eligible sampling strata is empty")


def sampling_capacity_report(source_index: Sequence[dict[str, Any]]) -> dict[str, Any]:
    """Report per-layer primary plus two-reserve capacity without double counting claims."""
    counts = {
        stratum: sum(
            stratum in (row.get("eligible_sampling_strata") or []) for row in source_index
        )
        for stratum in SAMPLING_STRATA_CANONICAL_ORDER
    }
    required = {
        stratum: FORMAL_SAMPLING_QUOTAS[stratum] * (1 + FORMAL_RESERVE_BLOCK_COUNT)
        for stratum in SAMPLING_STRATA_CANONICAL_ORDER
    }
    layers = {
        stratum: {
            "eligible_cases": counts[stratum],
            "required_cases": required[stratum],
            "capacity_pass": counts[stratum] >= required[stratum],
        }
        for stratum in SAMPLING_STRATA_CANONICAL_ORDER
    }
    return {
        "reserve_block_count": FORMAL_RESERVE_BLOCK_COUNT,
        "required_block_multiplier": 1 + FORMAL_RESERVE_BLOCK_COUNT,
        "layers": layers,
        "per_layer_capacity_pass": all(row["capacity_pass"] for row in layers.values()),
        "global_case_disjoint_allocation_verified": False,
        "global_case_disjoint_allocation_note": (
            "Per-layer reachability is necessary but not sufficient; the versioned selector "
            "must separately prove a case-disjoint global allocation."
        ),
    }
...
def build_review_point_manifest(
    *,
    ...
    return {
        "manifest_version": "hybrid-v3-review-points-v4",
        "builder_version": BUILDER_VERSION,
        ...
        **sampling_contract_pin,
        ...
        "expected_candidate3_review_points": expected_review_points,
        "universe_scope": "FULL_CANDIDATE3_REVIEW_POINT_UNIVERSE",
        "sampling_strata_basis": "OBJECTIVE_ONLY_AS_OF_MULTI_LABEL",
        "sampling_strata_canonical_order": list(SAMPLING_STRATA_CANONICAL_ORDER),
        "eligible_sampling_stratum_counts": {
            stratum: sum(
                stratum in row["eligible_sampling_strata"] for row in source_index
            )
            for stratum in SAMPLING_STRATA_CANONICAL_ORDER
        },
        "primary_sampling_focus_counts": dict(
            sorted(Counter(str(row["primary_sampling_focus"]) for row in source_index).items())
        ),
        "sampling_capacity": sampling_capacity_report(source_index),
        "source_manifest": {
...
def merge_formal_build_shards(
    *,
    ...
    for shard_id in range(FORMAL_BUILD_SHARD_COUNT):
        ...
        for field, value in _sampling_contract_pin().items():
            if shard_manifest.get(field) != value:
                raise FormalBuildIntegrityError(
                    f"formal shard manifest changed sampling contract pin: {field}"
                )
    ...
    for source_ordinal, row in enumerate(wrapper_rows):
        eligible = eligible_sampling_strata(row["packet"])
        source_index.append(
            {
                ...
                "eligible_sampling_strata_sha256": canonical_sha256(eligible),
                "primary_sampling_focus": primary_sampling_focus(eligible),
