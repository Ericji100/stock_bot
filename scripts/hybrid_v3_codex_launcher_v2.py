"""Frozen launcher for transport-hardened outcome-blind V3 atomic review."""
...
LAUNCHER_VERSION = "hybrid-v3-codex-launcher-v2-exact-evidence"
LAUNCHER_STATUS = "FINAL"
POLICY_PATH = ROOT / "scripts/hybrid_v3_atomic_policy_v3.py"
REVIEWER_PATH = ROOT / "scripts/hybrid_v3_codex_reviewer_v3.py"
...
def _verify_launcher_pin(execution_contract_path: Path) -> str:
    ...
    actual_sha = reviewer_v3.v2._file_sha256(actual_path)
...
def run_from_args(args: Any) -> dict[str, Any]:
    records = reviewer_v3.v2._read_jsonl(args.case_records)
    ...
    verified = reviewer_v3.verify_execution_contract(
        records,
    ...
    reviewer = reviewer_v3.CodexAtomicReviewerV3(
        prompt_path=args.prompt,
        schema_path=args.schema,
        model=args.model,
    ...
    runner = AtomicCaseRunner(
        ...
        expected_reviewer_identity={
            "adapter_version": reviewer_v3.ADAPTER_VERSION,
            "adapter_status": reviewer_v3.ADAPTER_STATUS,
...
def main() -> int:
    ...
    parser.add_argument("--model", default=reviewer_v3.DEFAULT_MODEL)
    parser.add_argument("--reasoning-effort", default=reviewer_v3.DEFAULT_REASONING)
    parser.add_argument("--protocol", type=Path, required=True)
    parser.add_argument("--prompt", type=Path, default=reviewer_v3.PROMPT_PATH)
    parser.add_argument("--schema", type=Path, default=reviewer_v3.SCHEMA_PATH)
