"""Audit the three-run exclusive-mature V4-S1 diagnostic.

The report is outcome blind and diagnostic only.  It validates the immutable
case/envelope identities, applies the frozen V4-S1 reducer, and measures exact
agreement.  It never opens identity maps, future prices, or performance data.
"""
...
VERSION = "hybrid-v4-s1-clean-positive-report-v1"
...
def _run_map(
    ...
    for path in sorted((Path(root) / "cases").glob("*.json")):
        ...
        if path.stem != envelope.get("run_case_key"):
            raise ValueError(f"run envelope filename changed: {path}")
...
def evaluate(
    *,
    ...
    if len(records) != 2 or len(run_roots) != 3:
        raise ValueError("diagnostic requires exactly two cases and three runs")
    runs = [
    ...
    unanimous_trades = 0
    exact_permission_cases = 0
    exact_scenario_stage_cases = 0
        ...
        scenario_stage_exact = len(
            {
                (
                    signature["scenario"],
                    signature["stage"],
                    signature["left_right_phase"],
                )
                for signature in signatures
            }
        ) == 1
        exact_permission_cases += int(permission_exact)
        exact_scenario_stage_cases += int(scenario_stage_exact)
    ...
    report_core = {
        ...
        "status": "DIAGNOSTIC_NO_UNANIMOUS_TRADE"
        if unanimous_trades == 0
        else "DIAGNOSTIC_HAS_UNANIMOUS_TRADE",
        "status_zh": "兩案均未形成三輪一致進場"
        if unanimous_trades == 0
        else "至少一案形成三輪一致進場",
        "classification": "OUTCOME_BLIND_DIAGNOSTIC_NOT_FORMAL_ACCEPTANCE",
        "cases": len(records),
        "runs": len(runs),
        ...
        "metrics": {
            ...
            "material_permission_exact": exact_permission_cases / len(records),
            "scenario_stage_phase_exact": exact_scenario_stage_cases / len(records),
            "all_atomic_answers": {
                "exact": pooled_exact,
                "total": pooled_total,
        ...
        "per_case": per_case,
        "interpretation": [
            "The two exclusive structural candidates are not semantic gold labels.",
            "Zero trades does not prove V4-S1 can never trade.",
            "The result supports calibrating local-stage and exhaustion atoms before a new formal sample.",
        ],
    }
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# V4-S1 乾淨成熟候選三輪診斷",
        "",
        f"- 結果：`{report['status']}`（{report['status_zh']}）",
        "- 性質：結果盲化診斷，不是正式驗收，也不是績效報告。",
    ...
    lines.extend(
        [
            ...
            "## 解讀",
            "",
            "兩案在交易權限上都穩定為 `WAIT（等待）`，但其中一案的局部位階在 `UNRESOLVED（未決）` 與 `EXHAUSTED（耗竭）` 間變動。這表示目前主要問題已從抽樣衝突縮小到局部位階與耗竭原子的判讀一致性。",
            "",
            "這兩案不是課程標準答案，因此零進場不能推論 V4-S1 永遠不會進場；正式重驗應先版本化澄清低一致原子，再用修正後的互斥 36 案抽樣。",
            "",
        ]
    )
    return "\n".join(lines)
