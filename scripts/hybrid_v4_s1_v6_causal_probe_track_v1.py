"""Freeze one outcome-blind V4-S1 probe from the V6 causal packet builder."""
...
VERSION = "hybrid-v4-s1-v6-causal-probe-track-v1"
...
def _eligible(packet: dict[str, Any]) -> bool:
    ...
    if objective.get("builder_version") != "hybrid-v3-atomic-packets-v6":
        return False
    if objective.get("wait_boundary_reasons"):
        return False
    ...
    if route.get("status") is not True or len(hypotheses) != 1:
        return False
    hypothesis = hypotheses[0]
    return all(
        hypothesis.get(field)
        for field in ("anchor_ref", "relation_ref", "episode_stop_ref", "campaign_stop_ref")
    )
...
def prepare(source: Path, output_dir: Path) -> dict[str, Any]:
    ...
    if len(packets) != 1:
        raise RuntimeError(f"expected exactly one V6 causal probe, found {len(packets)}")
    packet = packets[0]
    ...
    packet_manifest_core = {
        ...
        "selection_reason": "V6_EXACT_CAUSAL_MATURE_ROUTE_SUFFICIENT_WITHOUT_WAIT_BOUNDARY",
    ...
    manifest_core = {
        ...
        "v6_packet_builder_sha256": file_sha256(
            ROOT / "scripts/hybrid_v3_atomic_packets_v6.py"
        ),
