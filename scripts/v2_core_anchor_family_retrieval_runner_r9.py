"""Immutable R9A family-retrieval runner bound to frozen R8 defense."""
...
from scripts.v2_core_anchor_family_retrieval_r9 import parse_raw_without_duplicate_keys, transport_for, validate_and_retrieve
...
VERSION = "v2-core-anchor-family-retrieval-runner-r9-candidate-r1"
PROMPT_FILE = "v2_core_anchor_family_retrieval.prompt.candidate_r9.md"
MANIFEST_DIRECTORY = "anchor_family_retrieval_manifests_candidate_r9"
OUTPUT_DIRECTORY = "anchor_family_retrieval_runs_candidate_r9"
...
def _inputs(artifact_dir: Path, review_id: str) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if packet["review_id"] != review_id or groups != build_for_file(packet_path):
        raise RunnerError("R9A packet/groups changed")
    ...
    if defense_result["resolution_status"] != "SELECTED":
        raise RunnerError("R9A requires selected R8 defense")
    ...
    return packet, groups, defense_result, defense_row


def build_manifest(artifact_dir: Path, review_id: str) -> dict[str, Any]:
    packet, groups, defense_result, _defense_row = _inputs(artifact_dir, review_id)
    defense_id = defense_result["selected_candidate_id"]
    schema = transport_for(packet, groups, defense_id)
    defense_output, defense_receipt = defense_paths(artifact_dir, review_id)
    contract = ROOT / "scripts" / "v2_core_anchor_family_retrieval_r9.py"
    generator = ROOT / "scripts" / "v2_core_anchor_basis_groups_r9.py"
    return {
        ...
        "groups_sha256": sha256_path(artifact_dir / GROUP_DIRECTORY / f"{review_id}.json"),
        "group_generator_sha256": sha256_path(generator),
...
def _bound_manifest(artifact_dir: Path, review_id: str) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if stored != build_manifest(artifact_dir, review_id):
        raise RunnerError("R9A manifest changed")
    packet, groups, _defense_result, defense_row = _inputs(artifact_dir, review_id)
    schema = transport_for(packet, groups, stored["bound_defense_candidate_id"])
    return stored, packet, groups, defense_row, schema


def rendered_prompt(prompt_text: str, packet: dict[str, Any], groups: dict[str, Any], defense_row: dict[str, Any]) -> str:
    payload = {"packet": packet, "frozen_trade_episode_defense": defense_row, "basis_groups": groups["groups"]}
    return (
        prompt_text.rstrip()
        + "\n\n以下是匿名AS-OF封包、固定防線與必須逐組提名的basis族群。只做族群檢索。\n"
...
def run_retrieval(
    *, artifact_dir: Path, review_id: str, usage_attestation_path: Path,
    ...
    manifest, packet, groups, defense_row, schema = _bound_manifest(artifact_dir, review_id)
    ...
    if any(path.exists() for path in (output, receipt_path, raw, invalid)):
        raise RunnerError("R9A output exists; validate instead of re-running")
    base.assert_budget_allows(base.parse_usage_attestation(usage_attestation_path), manifest["stop_remaining_percent_lte"])
    prompt = rendered_prompt((artifact_dir / PROMPT_FILE).read_text(encoding="utf-8"), packet, groups, defense_row)
    ...
    try:
        ...
        raise RunnerError("R9A raw JSON duplicate/parse validation failed") from exc
    if duplicate_checked != response:
        base.write_new_or_identical(invalid, raw_bytes)
        raise RunnerError("R9A raw/transport mismatch")
    validation = validate_and_retrieve(
        response, packet=packet, groups=groups,
        defense_id=manifest["bound_defense_candidate_id"],
    )
    if validation["status"] != "VALID":
        base.write_new_or_identical(invalid, raw_bytes)
        raise RunnerError("R9A local validation failed: " + json.dumps(validation, ensure_ascii=False))
    receipt = {
        **receipt,
        "runner_version": VERSION,
        "stage": "R9A_ANCHOR_FAMILY_RETRIEVAL_CANDIDATE",
        "review_id": review_id,
        "as_of": packet["as_of"],
...
def validate_existing(*, artifact_dir: Path, review_id: str) -> dict[str, Any]:
    ...
    if not all(path.is_file() for path in (output, receipt_path, raw)):
        raise RunnerError("R9A output/raw/receipt incomplete")
    response = load_json(output)
    if parse_raw_without_duplicate_keys(raw.read_bytes()) != response:
        raise RunnerError("R9A raw/normalized mismatch")
    validation = validate_and_retrieve(
        response, packet=packet, groups=groups,
        defense_id=manifest["bound_defense_candidate_id"],
    )
    if validation["status"] != "VALID":
        raise RunnerError("R9A existing output invalid")
    prompt = rendered_prompt((artifact_dir / PROMPT_FILE).read_text(encoding="utf-8"), packet, groups, defense_row)
    ...
    for key, expected in checks.items():
        if receipt.get(key) != expected:
            raise RunnerError(f"R9A receipt mismatch: {key}")
    if receipt.get("validation") != validation:
        raise RunnerError("R9A receipt validation mismatch")
    return validation
