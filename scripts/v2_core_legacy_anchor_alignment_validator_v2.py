"""Validate R2 one-pass legacy controlling-anchor alignment responses.

R2 preserves an AI preferred anchor even when an alternative would change the
control conclusion.  The program then deterministically downgrades that
perception to an unresolved, non-routable state instead of rejecting it.
"""
...
def validate_response(
    *, response: dict[str, Any], schema: dict[str, Any], packet: dict[str, Any]
    ...
    changes_conclusion = selection.get("alternative_changes_control_conclusion")
    if changes_conclusion is True and not alternatives:
        errors.append("permission-changing alternative requires an alternative candidate")
    ...
    if status == "SELECTED":
        ...
        if alternatives:
            errors.append("UNRESOLVED cannot retain alternative candidates")
        if changes_conclusion is True:
            errors.append("UNRESOLVED cannot assert an alternative changes the conclusion")
        if scenario != "UNRESOLVED_NO_TRADE":
    ...
    ai_selected_candidate = candidates.get(selected_id)
    if status == "UNRESOLVED":
        program_status = "UNRESOLVED_AI"
        program_candidate = None
        program_scenario = "UNRESOLVED_NO_TRADE"
    elif changes_conclusion is True:
        program_status = "UNRESOLVED_CONFLICT"
        program_candidate = None
        program_scenario = "UNRESOLVED_NO_TRADE"
    else:
        program_status = "SELECTED"
        program_candidate = ai_selected_candidate
    ...
    return {
        ...
        "as_of": packet["as_of"],
        "ai_selection_status": status,
        "ai_selected_candidate": ai_selected_candidate,
        "ai_recommended_scenario_family": scenario,
        "alternative_candidate_ids": alternatives,
        "alternative_changes_control_conclusion": changes_conclusion,
        "program_usable_selection_status": program_status,
        "program_usable_candidate": program_candidate,
        "program_usable_scenario_family": program_scenario,
        "validator_contract": {
            "ai_selection_repaired": False,
            "ai_selection_upgraded": False,
            "candidate_dates_bound_by_program": True,
            "conflict_downgrade_is_deterministic": True,
