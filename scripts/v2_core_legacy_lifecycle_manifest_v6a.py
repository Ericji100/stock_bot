"""Freeze the R6A transport-only legacy lifecycle execution manifest."""
...
MANIFEST_VERSION = "v2-core-legacy-lifecycle-execution-r6a-candidate"
OUTPUT_FILE = "legacy_lifecycle_execution_manifest_candidate_r6a.json"
EXPECTED_PRIOR_R6_SHA256 = (
    "1303c73717a82e844af20dc341a4e4e3d15640fdf639feed2215c751d96a3210"
)
...
def build_manifest(artifact_dir: Path) -> dict[str, Any]:
    files = {
        ...
        "runner": ROOT / "scripts" / "v2_core_legacy_lifecycle_runner_v6a.py",
    ...
    prior = load_json(files["prior_r6_execution_manifest"])
    prior_hash = sha256_path(files["prior_r6_execution_manifest"])
    if prior_hash != EXPECTED_PRIOR_R6_SHA256:
        raise ValueError("prior R6 execution manifest hash changed")
    if prior.get("manifest_version") != "v2-core-legacy-lifecycle-execution-r6-candidate":
        raise ValueError("unexpected prior R6 manifest version")
    ...
    for key in unchanged_keys:
        if sha256_path(files[key]) != prior.get(f"{key}_sha256"):
            raise ValueError(f"R6 semantic or policy artifact changed: {key}")
    if sha256_path(files["base_runner"]) != prior.get("runner_sha256"):
    ...
    manifest: dict[str, Any] = {
        ...
        "subgate": "R6A_TRANSPORT_ONLY_REVISION",
        ...
        "run_directory": "legacy_lifecycle_runs_candidate_r6a",
        "prior_r6_status": "INVALIDATED_PRE_INFERENCE_TRANSPORT",
        "prior_r6_semantic_output_count": 0,
        "transport_only_revision": True,
        "transport_revision": "INLINE_DYNAMIC_ARRAY_ITEMS_AND_FORBID_REF_SIBLINGS",
    ...
    script_keys = {"runner", "base_runner", "b0a_validator", "b0b_validator"}
        ...
        json.dumps(
            {
                ...
                "transport_only_revision": manifest["transport_only_revision"],
