"""Per-ambiguity R7 AI adjudication with frozen qualification and receipts."""
...
from scripts.v2_core_legacy_role_adjudication_r7 import transport_for, validate_and_resolve
from scripts.v2_core_legacy_role_qualification_manifest_r7 import BUDGET_FILE, MODEL, OUTPUT_MANIFEST, REASONING
from scripts.v2_core_legacy_role_qualification_runner_r7 import (
    _bound_inputs,
    output_paths as qualification_paths,
    validate_existing as validate_qualification_existing,
)
from scripts.v2_core_legacy_role_shortlist_r7 import ARTIFACT_DIR, canonical_bytes, load_json, sha256_path


VERSION = "v2-core-legacy-role-adjudication-runner-r7-candidate-r1"
PROMPT_FILE = "v2_core_legacy_role_adjudication.prompt.candidate_r7.md"
MANIFEST_DIRECTORY = "legacy_role_adjudication_manifests_candidate_r7"
OUTPUT_DIRECTORY = "legacy_role_adjudication_runs_candidate_r7"
...
def _qualification_context(artifact_dir: Path, review_id: str, role: str) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    qualification = validate_qualification_existing(artifact_dir=artifact_dir, review_id=review_id, role=role)
    if qualification["resolution_reason"] != "MULTIPLE_NON_EQUIVALENT_ELIGIBLE_CANDIDATES":
        raise RunnerError("R7 adjudication requires frozen non-equivalent ambiguity")
    source_manifest, row, packet, shortlist = _bound_inputs(artifact_dir, review_id, role)
    return source_manifest, row, packet, shortlist, qualification


def build_manifest(artifact_dir: Path, review_id: str, role: str) -> dict[str, Any]:
    _source_manifest, row, packet, shortlist, qualification = _qualification_context(artifact_dir, review_id, role)
    qualified_output, qualified_receipt = qualification_paths(artifact_dir, review_id, role)
    qualified_raw = base.raw_output_path(qualified_output)
    schema = transport_for(packet, shortlist, role, qualification)
    prompt_path = artifact_dir / PROMPT_FILE
    contract_path = ROOT / "scripts" / "v2_core_legacy_role_adjudication_r7.py"
    return {
        ...
        "required_rounds": 1,
        "source_execution_manifest_sha256": sha256_path(artifact_dir / OUTPUT_MANIFEST),
        "input_packet_sha256": row["input_packet_sha256"],
        "shortlist_sha256": row["shortlist_sha256"],
        "qualification_output_sha256": sha256_path(qualified_output),
        "qualification_raw_sha256": sha256_path(qualified_raw),
        "qualification_receipt_sha256": sha256_path(qualified_receipt),
        "eligible_candidate_ids": qualification["eligible_candidate_ids"],
        "prompt_file": PROMPT_FILE,
        "prompt_sha256": sha256_path(prompt_path),
        "adjudication_contract_file": contract_path.name,
        "adjudication_contract_sha256": sha256_path(contract_path),
...
def _bound_manifest(artifact_dir: Path, review_id: str, role: str) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    expected = build_manifest(artifact_dir, review_id, role)
    path = manifest_path(artifact_dir, review_id, role)
    stored = load_json(path)
    if stored != expected:
        raise RunnerError("R7 adjudication manifest bindings changed")
    _source_manifest, _row, packet, shortlist, qualification = _qualification_context(artifact_dir, review_id, role)
    return stored, packet, shortlist, qualification


def rendered_prompt(prompt_text: str, packet: dict[str, Any], shortlist: dict[str, Any], role: str, qualification: dict[str, Any], qualification_output: dict[str, Any]) -> str:
    ...
    return (
        prompt_text.rstrip()
        + "\n\n以下是匿名AS-OF封包與第一階段凍結的合格候選。只比較eligible，不改評資格。\n"
...
def run_adjudication(
    *,
    ...
    manifest, packet, shortlist, qualification = _bound_manifest(artifact_dir, review_id, role)
    output, receipt_path = output_paths(artifact_dir, review_id, role)
    raw = base.raw_output_path(output)
    invalid = base.invalid_output_path(output)
    if any(path.exists() for path in (output, receipt_path, raw, invalid)):
        raise RunnerError("R7 adjudication output exists; validate instead of re-running")
    ...
    prompt = rendered_prompt(
        ...
        packet, shortlist, role, qualification, load_json(qualified_output),
    )
    ...
    validation = validate_and_resolve(response, packet=packet, shortlist=shortlist, role=role, qualification=qualification)
    if validation["status"] != "VALID":
        base.write_new_or_identical(invalid, raw_bytes)
        raise RunnerError("R7 adjudication local validation failed: " + json.dumps(validation, ensure_ascii=False))
    receipt = {
        **receipt,
        "runner_version": VERSION,
        "stage": "R7_ROLE_ADJUDICATION",
        "review_id": review_id,
        "as_of": packet["as_of"],
        "role": role,
        "adjudication_manifest_sha256": sha256_path(manifest_path(artifact_dir, review_id, role)),
        "qualification_output_sha256": manifest["qualification_output_sha256"],
        "prompt_source_sha256": manifest["prompt_sha256"],
        "adjudication_contract_sha256": manifest["adjudication_contract_sha256"],
...
def validate_existing(*, artifact_dir: Path, review_id: str, role: str) -> dict[str, Any]:
    ...
    if not all(path.is_file() for path in (output, receipt_path, raw)):
        raise RunnerError("R7 adjudication output/raw/receipt incomplete")
    response = load_json(output)
    validation = validate_and_resolve(response, packet=packet, shortlist=shortlist, role=role, qualification=qualification)
    if validation["status"] != "VALID":
        raise RunnerError("R7 existing adjudication invalid")
    qualified_output, _qualified_receipt = qualification_paths(artifact_dir, review_id, role)
    schema = transport_for(packet, shortlist, role, qualification)
    prompt = rendered_prompt(
        (artifact_dir / PROMPT_FILE).read_text(encoding="utf-8"),
    ...
    for key, expected in checks.items():
        if receipt.get(key) != expected:
            raise RunnerError(f"R7 adjudication receipt mismatch: {key}")
    if receipt.get("validation") != validation:
        raise RunnerError("R7 adjudication validation receipt mismatch")
    return validation
