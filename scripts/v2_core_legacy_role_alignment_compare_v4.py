"""Reveal-bound comparator for the frozen R4 structural-role run."""
...
from scripts.v2_core_legacy_role_alignment_validator_v4 import load_json


ARTIFACT_DIR = (
    ROOT / "reports" / "course_backtest" / "2026-09-10" / "v2_core_reproducible_goal_v1"
)
REPORT_VERSION = "v2-core-legacy-role-alignment-report-r4-candidate"
...
def build_report(artifact_dir: Path) -> dict[str, Any]:
    ...
    for row in execution["rows"]:
        ...
        receipt_path = run_dir / "receipts" / f"{review_id}.legacy_role_alignment_r4.json"
    ...
    if missing or invalid or completed != expected:
        ...
        report["required_next_action"] = (
            "Complete immutable R4 outputs; calibration references remain unopened."
        )
        return report
        ...
        cases.append(
            {
                ...
                "parent_status": validation["role_statuses"]["PARENT"],
    ...
    report.update(
        {
            ...
            "required_next_action": (
                "Build the single-pass trigger/episode-stop/permission alignment stage; do not start triplicate consistency yet."
                if passed
...
def main() -> int:
    ...
    json_path = args.artifact_dir / "legacy_role_alignment_report_candidate_r4.json"
    md_path = args.artifact_dir / "legacy_role_alignment_report_candidate_r4.md"
