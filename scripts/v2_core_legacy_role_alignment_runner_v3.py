"""Immutable, usage-guarded runner for R3 legacy role alignment."""
...
from scripts.v2_core_legacy_role_alignment_validator_v3 import load_json, validate_response


RUNNER_VERSION = "v2-core-legacy-role-alignment-runner-r3-candidate"
MODEL = "gpt-5.6-sol"
REASONING = "xhigh"
...
def run_case(
    *,
    ...
    if validation["status"] != "VALID":
        ...
        raise RunnerError(
            "LEGACY_ROLE_ALIGNMENT_R3 local validation failed: "
    ...
    bound = {
        **receipt,
        "stage": "LEGACY_ROLE_ALIGNMENT_B0_R3",
...
def validate_existing_artifacts(
    *,
    ...
    if not output_path.is_file() or not raw_path.is_file() or not receipt_path.is_file():
        raise RunnerError("existing legacy role R3 output/raw/receipt set is incomplete")
    packet = load_json(input_packet_path)
    schema = load_json(schema_path)
    _manifest_row(packet, input_packet_path, input_manifest_path)
    ...
    validation = validate_response(response=output, schema=schema, packet=packet)
    if validation["status"] != "VALID":
        raise RunnerError("existing legacy role R3 output is invalid")
    ...
    for key, expected in checks.items():
        if receipt.get(key) != expected:
            raise RunnerError(f"existing legacy role R3 receipt mismatch: {key}")
    if receipt.get("validation") != validation:
        raise RunnerError("existing legacy role R3 receipt validation mismatch")
    return validation
