"""Apply a stricter, versioned operational budget guard around the frozen v6 runner."""
...
GUARD_VERSION = "v2-core-operational-budget-guard-r1"
EXPECTED_POLICY_VERSION = "v2-core-operational-budget-override-v2"
...
def load_policy(path: Path) -> dict[str, Any]:
    ...
    if value["policy_version"] != EXPECTED_POLICY_VERSION or value["status"] != "ACTIVE":
        raise StagedRunnerError("operational budget policy is not the active v2 override")
    if value["usage_source"] != "CODEX_APP_GET_USAGE_LIMITS":
        raise StagedRunnerError("operational budget source mismatch")
    if value["usage_limit_id"] != "codex":
        raise StagedRunnerError("operational budget limit mismatch")
...
def runner_command(args: argparse.Namespace) -> list[str]:
    return [
        sys.executable,
        str(ROOT / "scripts" / "v2_core_codex_staged_runner_v1.py"),
        "--artifact-dir",
