"""Immutable Codex runner for focused Stage B1a1E evidence perceptions."""
...
RUNNER_VERSION = "v2-core-stage-b1a1e-codex-runner-r1-candidate"
MODEL = base.MODEL
REASONING = base.REASONING
StageB1a1ERunnerError = base.StageB1aRunnerError
RunCommand = Callable[..., subprocess.CompletedProcess[str]]


def _assert_input_manifest_binding(
    *, packet: dict[str, Any], input_packet_path: Path, input_manifest_path: Path
    ...
    if manifest.get("formal_model") != MODEL or manifest.get("reasoning_effort") != REASONING:
        raise StageB1a1ERunnerError("input manifest model/effort mismatch")
    matches = [
        row for row in manifest["rows"] if row["review_id"] == packet["review_id"]
    ]
    if len(matches) != 1:
        raise StageB1a1ERunnerError("input manifest review_id mismatch")
    row = matches[0]
    if row["as_of"] != packet["as_of"]:
        raise StageB1a1ERunnerError("input manifest as_of mismatch")
    if row["input_packet_file"] != input_packet_path.name:
        raise StageB1a1ERunnerError("input manifest filename mismatch")
    if row["input_packet_sha256"] != sha256_file(input_packet_path):
        raise StageB1a1ERunnerError("input manifest packet hash mismatch")
...
def run_stage_b1a1e_case(
    *,
    ...
    _assert_input_manifest_binding(
        packet=packet,
        input_packet_path=input_packet_path,
        input_manifest_path=input_manifest_path,
    )
    option_ids = [row["evidence_option_id"] for row in packet["evidence_options"]]
    ...
    validation = validate_stage_b1a1e_response(
        response=output,
        schema=schema,
        packet=packet,
    )
    if validation["status"] != "VALID":
        ...
        raise StageB1a1ERunnerError(
            "STAGE_B1A1E local validation failed: "
...
def validate_existing_stage_b1a1e_artifacts(
    *,
    input_packet_path: Path,
    input_manifest_path: Path,
    schema_path: Path,
    ...
    if not output_path.is_file() or not raw_path.is_file() or not receipt_path.is_file():
        raise StageB1a1ERunnerError("existing B1a1E output/raw/receipt set is incomplete")
    packet = load_json(input_packet_path)
    schema = load_json(schema_path)
    _assert_input_manifest_binding(
        packet=packet,
    ...
    if validation["status"] != "VALID":
        raise StageB1a1ERunnerError("existing B1a1E output is invalid")
    ...
    for key, expected in checks.items():
        if receipt.get(key) != expected:
            raise StageB1a1ERunnerError(f"existing B1a1E receipt mismatch: {key}")
    if receipt.get("validation") != validation:
        raise StageB1a1ERunnerError("existing B1a1E receipt validation mismatch")
    return validation
...
def main() -> int:
    args = parse_args()
    validation = run_stage_b1a1e_case(
