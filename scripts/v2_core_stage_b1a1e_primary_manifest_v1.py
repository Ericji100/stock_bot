"""Build immutable independent execution manifest for B1a1E candidate R3."""
...
from scripts.v2_core_stage_b1a1e_primary_input_v1 import generate_all


OUTPUT_FILE = "stage_b1a1e_primary_execution_manifest_candidate_r3.json"
OUTPUT_MD = "stage_b1a1e_primary_execution_manifest_candidate_r3.md"


def build_manifest(artifact_dir: Path = ARTIFACT_DIR) -> dict:
    input_manifest = generate_all(artifact_dir)
    bindings = {
        "input_manifest_file": "stage_b1a1e_primary_input_manifest_candidate_r3.json",
        ...
        "gate_file": "B1A1E_NEXT_CANDIDATE_DESIGN_R3.md",
        "operational_policy_file": "operational_budget_override_v11.json",
    }
    manifest = {
        "manifest_version": "v2-core-stage-b1a1e-primary-anchor-execution-r3-candidate",
        "status": "READY_FOR_FORMAL_AI_INDEPENDENT_PILOT（可執行正式AI獨立試跑）",
        ...
        "run_directory": "stage_b1a1e_primary_runs_candidate_r3",
    ...
    write_new_or_identical(
        ...
        (
            "# Stage B1a1E主錨候選R3獨立試跑執行清單\n\n"
            f"- 狀態：`{manifest['status']}`\n"
            f"- 模型：`{manifest['formal_model']}／{manifest['reasoning_effort']}`\n"
            f"- 全新案例輪次：{manifest['expected_case_rounds']}\n"
            "- 主候選只含LARGE／SMALL；AUXILIARY不得獨立進池。\n"
