"""Build immutable execution manifest for Stage B1a1E focused smoke R1."""
...
from scripts.v2_core_stage_b1a1e_focus_input_v1 import generate_all, load_json


OUTPUT_FILE = "stage_b1a1e_smoke_execution_manifest_candidate_r1.json"
OUTPUT_MD = "stage_b1a1e_smoke_execution_manifest_candidate_r1.md"


def build_manifest(artifact_dir: Path = ARTIFACT_DIR) -> dict:
    input_manifest = generate_all(artifact_dir)
    bindings = {
        "input_manifest_file": "stage_b1a1e_input_manifest_candidate_r1.json",
        "prompt_file": "v2_core_stage_b1a1e.prompt.candidate_r1.md",
        "schema_file": "v2_core_stage_b1a1e.schema.candidate_r1.json",
        ...
        "gate_file": "B1A1E_SMOKE_GATE_R1.md",
        "operational_policy_file": "operational_budget_override_v10.json",
    }
    manifest = {
        "manifest_version": "v2-core-stage-b1a1e-smoke-execution-r1-candidate",
        ...
        "run_directory": "stage_b1a1e_smoke_runs_candidate_r1",
    ...
    lines = [
        "# Stage B1a1E Smoke執行清單 R1",
        "",
        f"- 狀態：`{manifest['status']}`",
        f"- 正式模型：`{manifest['formal_model']}／{manifest['reasoning_effort']}`",
        f"- 案例：{manifest['case_count']}，輪次：{manifest['required_rounds']}，總呼叫：{manifest['expected_case_rounds']}",
        f"- 停止線：主要額度剩餘≤{manifest['stop_remaining_percent_lte']}%。",
        "- 舊R4答案未放入AI封包；未使用股票身分、sealed標籤或未來績效。",
    ]
    write_new_or_identical(
...
def main() -> int:
    manifest = build_manifest(parse_args().artifact_dir)
