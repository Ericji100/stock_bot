"""Build an answer-blind held-out Stage B1a1E validation pilot.

The review cases and segment candidates are selected only from stable hashes
and objective scale/direction buckets.  Calibration answers, sealed labels,
identity, and future performance are never consulted.
"""
...
SELECTION_SEED = "B1A1E-R2-HELDOUT-PILOT-V1"
CASE_COUNT = 4
MAX_CANDIDATES_PER_CASE = 6
CALIBRATION_MANIFEST = "stage_b1a1e_smoke_execution_manifest_candidate_r2.json"
SOURCE_INPUT_MANIFEST = "stage_b1a1_input_manifest_candidate_r2.json"
OUTPUT_DIRECTORY = "stage_b1a1e_validation_input_packets_r1"
OUTPUT_MANIFEST = "stage_b1a1e_validation_input_manifest_r1.json"
OUTPUT_SELECTION = "stage_b1a1e_validation_selection_r1.json"
PACKET_VERSION = "v2-core-stage-b1a1e-heldout-validation-input-r1"


def _digest(*parts: str) -> str:
    value = "|".join((SELECTION_SEED, *parts)).encode("utf-8")
...
def _select_reviews(source_rows: list[dict[str, Any]], excluded: set[str]) -> list[str]:
    ...
    if len(eligible) < CASE_COUNT:
        raise ValueError("insufficient held-out review cases")
    return sorted(eligible, key=lambda review_id: (_digest("CASE", review_id), review_id))[
        :CASE_COUNT
    ]
...
def _select_candidates(packet: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    segments = list(packet["focus_segments"])
    buckets: dict[tuple[str, str], list[dict[str, Any]]] = {}
    for segment in segments:
        key = (str(segment["scale"]), str(segment["direction"]))
    ...
    for key in sorted(buckets):
        winner = min(
            buckets[key],
            key=lambda row: (
                _digest("BUCKET", review_id, key[0], key[1], str(row["candidate_id"])),
    ...
    if len(selected) < MAX_CANDIDATES_PER_CASE:
        remaining = [
            row for row in segments if str(row["candidate_id"]) not in selected
        ]
        remaining.sort(
            key=lambda row: (
...
def generate_all(artifact_dir: Path = ARTIFACT_DIR) -> dict[str, Any]:
    ...
    calibration = load_json(artifact_dir / CALIBRATION_MANIFEST)
    excluded = {str(row["review_id"]) for row in calibration["rows"]}
    selected_reviews = _select_reviews(list(source_rows.values()), excluded)
    ...
    for review_id in selected_reviews:
        ...
        packet["status"] = "HELD_OUT_VALIDATION_INPUT_NOT_FORMALLY_RUN（留出驗證輸入、尚未正式執行）"
        packet["review_constraints"]["selection_uses_prior_ai_answers"] = False
        packet["review_constraints"]["held_out_from_r2_calibration"] = True
    ...
    selection = {
        "selection_version": "v2-core-stage-b1a1e-heldout-validation-selection-r1",
        "selection_seed": SELECTION_SEED,
        "selection_method": "SHA256 case sampling plus objective scale-direction stratification",
        "excluded_calibration_review_ids": sorted(excluded),
    ...
    manifest = {
        "manifest_version": "v2-core-stage-b1a1e-heldout-validation-input-r1",
        "status": "HELD_OUT_INPUTS_COMPLETE_NOT_READY_FOR_AI（留出輸入完成、尚不可執行AI）",
        ...
        "calibration_review_ids_excluded": True,
