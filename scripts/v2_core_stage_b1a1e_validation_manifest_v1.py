"""Build the immutable Stage B1a1E R2 held-out validation pilot manifest."""
...
from scripts.v2_core_stage_b1a1e_validation_input_v1 import generate_all


OUTPUT_FILE = "stage_b1a1e_validation_execution_manifest_r1.json"
OUTPUT_MD = "stage_b1a1e_validation_execution_manifest_r1.md"


def build_manifest(artifact_dir: Path = ARTIFACT_DIR) -> dict:
    input_manifest = generate_all(artifact_dir)
    bindings = {
        "input_manifest_file": "stage_b1a1e_validation_input_manifest_r1.json",
        ...
        "gate_file": "B1A1E_HELDOUT_VALIDATION_GATE_R1.md",
        "operational_policy_file": "operational_budget_override_v11.json",
    }
    manifest = {
        "manifest_version": "v2-core-stage-b1a1e-heldout-validation-execution-r1",
        "status": "READY_FOR_FORMAL_AI_VALIDATION_PILOT（可執行正式AI留出驗證試跑）",
        ...
        "run_directory": "stage_b1a1e_validation_runs_r1",
    ...
    write_new_or_identical(
        ...
        (
            ...
            "- 沿用R2 prompt、schema與程式真值表；未使用舊答案、未來績效、股票身分或sealed標籤。\n"
            "- 這是留出泛化試跑，不是完整M2A或交易績效。\n"
        ).encode("utf-8"),
    )
    return manifest
