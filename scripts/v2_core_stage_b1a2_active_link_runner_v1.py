"""Immutable Codex runner for the B1a2 active-campaign-link probe."""
...
RUNNER_VERSION = "v2-core-stage-b1a2-active-link-runner-r1-candidate"
MODEL = base.MODEL
...
def run_case(
    *, input_packet_path: Path, input_manifest_path: Path, schema_path: Path,
    prompt_path: Path, usage_attestation_path: Path, stop_remaining_percent_lte: float,
    output_path: Path, receipt_path: Path, timeout_seconds: int,
    ...
    _manifest_row(packet=packet, input_packet_path=input_packet_path, manifest_path=input_manifest_path)
    option_ids = [str(row["evidence_option_id"]) for row in packet["role_link_evidence_options"]]
    ...
    if validation["status"] != "VALID":
        ...
        raise RunnerError(
            "STAGE_B1A2_ACTIVE_LINK local validation failed: "
...
def validate_existing_artifacts(
    *, input_packet_path: Path, input_manifest_path: Path, schema_path: Path,
    prompt_path: Path, output_path: Path, receipt_path: Path,
) -> dict[str, Any]:
    raw_path = base.raw_output_path(output_path)
    if not output_path.is_file() or not raw_path.is_file() or not receipt_path.is_file():
        raise RunnerError("existing B1a2 output/raw/receipt set is incomplete")
    packet = load_json(input_packet_path)
    schema = load_json(schema_path)
    _manifest_row(packet=packet, input_packet_path=input_packet_path, manifest_path=input_manifest_path)
    ...
    validation = validate_response(response=output, schema=schema, packet=packet)
    if validation["status"] != "VALID":
        raise RunnerError("existing B1a2 output is invalid")
    ...
    for key, expected in checks.items():
        if receipt.get(key) != expected:
            raise RunnerError(f"existing B1a2 receipt mismatch: {key}")
    if receipt.get("validation") != validation:
        raise RunnerError("existing B1a2 receipt validation mismatch")
    return validation
