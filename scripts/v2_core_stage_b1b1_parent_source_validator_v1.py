"""Validate B1b1 parent-source perceptions and derive an eligibility pool."""
...
REASONS = {
    "SUPPORTS": {"COMPLETED_PREDECESSOR_GENERATES_CURRENT_CHAIN"},
    "CONTRADICTS": {"INTERNAL_OR_PARALLEL_NOT_SOURCE_PARENT", "VISIBLE_SUCCESSOR_CHAIN_SEVERS_PARENT_ROLE"},
    "INSUFFICIENT": {"FIXED_PATH_DOES_NOT_ESTABLISH_GENERATIVE_ROLE", "CONFLICTING_PARENT_PATHS"},
}
...
def validate_response(*, response: dict[str, Any], schema: dict[str, Any], packet: dict[str, Any]) -> dict[str, Any]:
    ...
    expected = {str(value) for value in packet["parent_candidate_ids"]}
    submitted = [str(row["candidate_id"]) for row in response.get("candidate_parent_perceptions", [])]
    ...
    options = {str(row["candidate_id"]): row for row in packet["parent_source_evidence_options"]}
    ...
    for candidate_id in expected:
        if segments[candidate_id]["status"] != "CONFIRMED" or options[candidate_id]["candidate_status"] != "CONFIRMED":
            errors.append(f"{candidate_id}:parent candidate is not confirmed")
        if options[candidate_id]["atom_name"] != "parent_source_relation":
    ...
    derived = []
    for row in response.get("candidate_parent_perceptions", []):
        candidate_id = str(row["candidate_id"])
        if candidate_id not in expected or candidate_id not in options:
            continue
        value = row["parent_source_relation"]
        ...
        if value["reason_code"] not in REASONS[value["judgement"]]:
        ...
        derived.append(
            {
                ...
                "parent_source_relation": {
                    ...
                    "primary_evidence_option_id": value["evidence_option_id"],
                },
                "parent_source_eligible": result == "PASS",
            }
        )
    if errors:
    ...
    return {
        ...
        "parent_source_eligible_ids": [row["candidate_id"] for row in derived if row["parent_source_eligible"]],
        "validator_contract": {
