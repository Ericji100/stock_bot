"""Freeze the M2A triplicate input matrix without calling an AI model."""
...
VERSION = "v2-core-m2a-triplicate-execution-r6-candidate"
MANIFEST_FILENAME = "feasibility_probe_execution_manifest_v6.json"
RUN_DIRECTORY = "feasibility_probe_runs_r6"
ROUNDS = (1, 2, 3)
SHARD_SIZE = 4
ARTIFACT_COMPONENT_FILES = (
    ...
    "v2_core_stage_d.prompt.candidate_r2.md",
...
def build(artifact_dir: Path) -> dict[str, Any]:
    ...
    rounds = [
        {
            ...
            "input_matrix_sha256": canonical_sha256(case_rows),
    ...
    return {
        ...
        "shards": shards,
        "rounds": rounds,
        "cross_round_input_matrix_sha256": canonical_sha256(case_rows),
        "run_directory": RUN_DIRECTORY,
        "resume_rule": "Never rerun an existing fully validated raw+normalized output+receipt+program artifact set. Preserve invalid raw output without repair; any retry requires a separately versioned attempt policy.",
    }
...
def validate(manifest: dict[str, Any], artifact_dir: Path) -> list[str]:
    ...
    if any(row.get("status") != "NOT_STARTED（尚未開始）" for row in manifest.get("rounds") or []):
        errors.append("UNEXPECTED_STARTED_ROUND")
    packet_dir = artifact_dir / json.loads(
        (artifact_dir / "feasibility_probe_manifest.json").read_text(encoding="utf-8")
    )["packet_directory"]
    for row in cases:
        ...
        json.dumps(
            {
                ...
                "cross_round_input_matrix_sha256": manifest[
                    "cross_round_input_matrix_sha256"
                ],
