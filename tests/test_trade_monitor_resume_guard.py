...
import trade_monitor_resume_guard as guard
...
def test_stale_completion_cannot_clear_newer_run_state(tmp_path):
    state_path = tmp_path / "runtime.json"
    guard.begin_monitor_run(state_path, now=NOW, run_id="run-1")
    guard.begin_monitor_run(state_path, now=NOW + timedelta(seconds=1), run_id="run-2")
