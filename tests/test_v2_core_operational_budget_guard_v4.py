...
from scripts import v2_core_operational_budget_guard_v3 as previous_guard
from scripts.v2_core_operational_budget_guard_v4 import main, preflight, runner_command
from scripts.v2_core_triplicate_preflight_v3 import OUT


MANIFEST = OUT / "feasibility_probe_execution_manifest_v8.json"
POLICY = OUT / "operational_budget_override_v5.json"
        ...
        json.dumps(
            {
                ...
                "check_id": f"v8-50pct-test-{remaining}",
            }
        ),
        encoding="utf-8",
    )
...
def test_v8_guard_enforces_latest_50_percent_threshold(tmp_path: Path) -> None:
    result = preflight(
        artifact_dir=OUT,
        manifest_path=MANIFEST,
        policy_path=POLICY,
        usage_attestation_path=_usage(tmp_path, 51),
    )
    assert result["remaining_percent"] == 51
    assert result["operational_stop_threshold_percent"] == 50
    with pytest.raises(StagedRunnerError, match="BUDGET_STOP"):
        preflight(
            artifact_dir=OUT,
            manifest_path=MANIFEST,
            policy_path=POLICY,
            usage_attestation_path=_usage(tmp_path, 50),
        )
...
def test_v8_guard_main_dispatches_without_changing_experiment(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(previous_guard.previous.subprocess, "run", fake_run)
    result = main(
        [
            "--artifact-dir",
            str(OUT),
