...
DEFAULT_STATE_PATH = Path(".runtime/trade_monitor/runtime_state.json")
...
def begin_monitor_run(
    ...
    *,
    ...
    if lease_seconds <= 0:
        raise BridgeError("lease_seconds_invalid", "lease_seconds must be positive")
    ...
    with DeliveryFileLock(lock_path):
        ...
        active_started = _parse_timestamp(state.get("last_started_at"))
        active_age = None if active_started is None else max(0.0, (current_time - active_started).total_seconds())
        if active_run_id and active_started is None:
            raise BridgeError("runtime_state_invalid", "Active trade monitor run has no lease timestamp")
        if active_run_id and active_age is not None and active_age < lease_seconds:
            return {
                "ok": True,
                "status": "busy",
                "run_id": None,
                ...
                "active_run_age_seconds": round(active_age, 3),
                "lease_seconds": lease_seconds,
            }

        stale_active_run = active_run_id if active_run_id else None
        ...
        force_notify = bool(stale_active_run) or pending or first_run or gap_exceeded

        if stale_active_run:
            reason = "active_run_lease_expired"
        elif pending:
        ...
        if stale_active_run:
            state["last_recovered_run_id"] = stale_active_run
            state["last_recovered_at"] = current_time.isoformat()
        state.update(
            {
    ...
    return {
        ...
        "lease_seconds": lease_seconds,
        "recovered_run_id": stale_active_run,
    }
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    begin_parser.add_argument("--lease-seconds", type=int, default=DEFAULT_LEASE_SECONDS)
