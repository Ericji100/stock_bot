...
def validate_analysis_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if actual != required:
        raise AnalysisValidationError(
            f"invalid top-level fields; missing={sorted(required - actual)}, extra={sorted(actual - required)}"
        )
    ...
    decision = _clean_text(payload["original_decision"])
...
def render_analysis_markdown(
    ...
    *,
    ...
    if new_bar_count > 1:
        time_lines.append(f"距上次成功處理新增約 {new_bar_count} 根已收盤 K；只重建目前結構，不回補歷史成交。")
    time_lines.extend(data["latest_closed_k_details"])

    output: list[str] = []
    output += _section("時間／最新已收盤 K", time_lines)
    output += _section("大趨勢", [f"**{data['large_trend']['classification']}**", *data["large_trend"]["details"]])
    output += _section("當前趨勢", [f"**{data['current_trend']['classification']}**", *data["current_trend"]["details"]])
    output += _section("市場狀態", data["market_state"])
    pattern = data["pattern_observation"]
    output += _section("觀察型態與狀態", [f"**{pattern['status']}：{pattern['pattern']}**", *pattern["details"]])
    ...
    output += _section("單口管理／禁止原因", data["single_contract_management_or_prohibition"])
